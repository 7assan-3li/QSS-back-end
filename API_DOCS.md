...
### 1.7 الموافقة على السياسات (Policy Acceptance) `PATCH /api/policies/{role}` (Auth)
**الوصف:** يُستخدم هذا المسار لتسديد الموافقة الفكرية والقانونية على سياسة الاستخدام المحدثة.
- **طلب طالب الخدمة:** `PATCH /api/policies/seeker`
- **طلب مزود الخدمة:** `PATCH /api/policies/provider`
...
### 3.7 إدارة وتحديث الخدمات التلقائية (اللقاء والمخصصة) (Provider Auth)
...
- **خدمة اللقاء (Meeting Service):**
  - استعراض بيانات الخدمة: `GET /api/services-meeting`
  - تعديل الأسعار والبيانات: `PUT /api/services-meeting`
- **الخدمة المخصصة (Custom Service):**
  - استعراض بيانات الخدمة: `GET /api/services-custom`
  - تعديل الأسعار والبيانات: `PUT /api/services-custom`
...
### 7.2 إرسال بلاغ حول طلب معين `POST /api/request-complaints` (Auth)
**الوصف:** في حال حصول خلاف أو عدم رضى عن طلب، يتم التصعيد عبر هذا المسار. **(متاح لكل من طالب الخدمة ومزود الخدمة المكلف بالطلب).**
- **البيانات:** 
  - `request_id` للطلب المراد الإبلاغ عنه.
  - `title` عنوان البلاغ.
  - `type` نوع أو تصنيف البلاغ.
  - `content` النص التفصيلي للمشكلة.
- **ملاحظة:** يتم ربط البلاغ بصاحب الـ Token تلقائياً، ويحدد النظام ما إذا كان "طالب خدمة" أو "مزود" بناءً على علاقته بالطلب.

### 7.3 إرسال شكوى عامة أو نظامية `POST /api/system-complaints` (Auth)
**الوصف:** إيصال مشكلة مرتبطة بالتطبيق نفسه (مثال: تعليق في نظام الإشعارات، مشكلة مالية، مقترح، إلخ).
- **البيانات:** 
  - `title` العنوان.
  - `type` تصنيف الشكوى.
  - `content` المحتوى.
  - `app_source` **(مهم جداً إرساله)** يجب أن يكون قيمته إما `"seeker"` (إذا كانت الشكوى من تطبيق المستفيد) أو `"provider"` (إذا كانت من تطبيق المزود).
