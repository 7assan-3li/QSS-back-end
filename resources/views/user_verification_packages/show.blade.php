    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-10 text-start font-Cairo">
        <div class="text-start font-Cairo">
            <div class="flex items-center gap-5 mb-5 text-start font-Cairo">
                <a href="{{ route('user-verification-packages.index') }}" class="w-14 h-14 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-[1.2rem] flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-sm border border-[var(--glass-border)] font-Cairo">
            ...
            <div class="flex items-center gap-5 text-start font-Cairo">
                <form id="approve-form-{{ $userPackage->id }}" action="{{ route('user-verification-packages.approve', $userPackage->id) }}" method="POST" class="font-Cairo">
                    ...
                    <button type="button" 
                        ...
                        })" class="px-10 py-5 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-[1.2rem] text-[14px] font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-600/20 hover:scale-[1.05] transition-all font-Cairo flex items-center gap-3 text-start font-Cairo">
                ...
                <form id="reject-form-{{ $userPackage->id }}" action="{{ route('user-verification-packages.reject', $userPackage->id) }}" method="POST" class="font-Cairo">
                    ...
                    <button type="button" 
                        onclick="promptRejection('reject-form-{{ $userPackage->id }}')" class="px-10 py-5 bg-rose-500/10 text-rose-600 border border-rose-500/20 rounded-[1.2rem] text-[14px] font-black uppercase tracking-[0.2em] hover:bg-rose-600 hover:text-white transition-all duration-500 font-Cairo flex items-center gap-3 text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 font-Cairo text-start">
        ...
        <div class="lg:col-span-8 space-y-12 text-start font-Cairo">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 text-start font-Cairo">
                ...
                <!-- Package Information -->
                <div class="card-premium glass-panel p-12 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo shadow-indigo-500/5">
                    ...
                    <div class="flex items-center gap-4 mb-12 text-start font-Cairo">
                        ...
                        <h4 class="text-xl font-black text-[var(--main-text)] font-Cairo text-start italic">{{ __('الباقة المطلوبة') }}</h4>
                    </div>
                    <div class="space-y-10 relative z-10 text-start font-Cairo">
                        <div class="flex items-center gap-6 group font-Cairo">
                            <div class="w-14 h-14 bg-[var(--main-bg)] text-brand-primary rounded-[1.2rem] flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition-transform font-Cairo italic">🏷️</div>
                            <div class="flex flex-col text-start font-Cairo">
                                <span class="text-[13px] font-black text-[var(--text-muted)] mb-2 font-Cairo text-start">{{ __('اسم الباقة') }}</span>
                        ...
                        <div class="flex items-center gap-6 group text-start font-Cairo">
                            <div class="w-14 h-14 bg-[var(--main-bg)] text-emerald-500 rounded-[1.2rem] flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition-transform font-Cairo italic">💰</div>
                            <div class="flex flex-col text-start font-Cairo">
                                <span class="text-sm font-black text-emerald-600 font-mono text-start font-Cairo">{{ $userPackage->verificationPackage ? number_format($userPackage->verificationPackage->price, 0) . ' ' . __('ريال يمني') : '--' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Payment Details -->
            <div class="card-premium glass-panel p-14 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo shadow-emerald-500/5">
                <div class="flex items-center justify-between mb-14 text-start font-Cairo">
                    <div class="flex items-center gap-5 text-start font-Cairo">
                        <span class="w-3 h-10 bg-slate-900 dark:bg-[var(--glass-bg)] rounded-full shadow-lg font-Cairo"></span>
                        <h4 class="text-2xl font-black text-[var(--main-text)] font-Cairo text-start italic">{{ __('بيانات الدفع وسند التحويل') }}</h4>
                    </div>
                    @php
                    ...
                    @endphp
                    <span class="px-8 py-3 rounded-[2rem] text-[14px] font-black uppercase tracking-[0.3em] shadow-sm border {{ $statusColor }} font-Cairo text-start">
                        {{ $statusLabel }}
                    </span>
                </div>

                <div class="p-12 bg-[var(--main-bg)] rounded-[1.5rem] border border-[var(--glass-border)] shadow-inner flex flex-col md:flex-row items-center gap-14 text-start font-Cairo shadow-brand-primary/5">
                    <div class="flex flex-col items-center gap-3 text-start font-Cairo">
                        <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] font-Cairo text-start">{{ __('رقم السند') }}</span>
                    ...
                    <div class="flex-1 text-center md:text-start space-y-4 font-Cairo">
                        @if($userPackage->admin)
                            <div class="flex flex-col gap-2 font-Cairo">
                                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('تمت المراجعة بواسطة') }}</span>
                                <span class="text-lg font-black text-[var(--main-text)] font-Cairo text-start">👤 {{ $userPackage->admin->name }}</span>
                            </div>
                            @if($userPackage->rejection_reason)
                                <div class="mt-4 p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl font-Cairo">
                                    <span class="text-[11px] font-black text-rose-500 uppercase tracking-widest block mb-1">{{ __('سبب الرفض') }}</span>
                                    <p class="text-sm font-bold text-rose-700 dark:text-rose-400 italic">"{{ $userPackage->rejection_reason }}"</p>
                                </div>
                            @endif
                        @else
                            <div class="flex flex-col gap-2 animate-pulse text-start font-Cairo font-Cairo">
                                <span class="text-[13px] font-black text-amber-500 uppercase tracking-[0.3em] font-Cairo text-start">{{ __('بانتظار المراجعة') }}</span>
        ...
        <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
            <div class="card-premium glass-panel p-10 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
                <div class="absolute bottom-0 left-0 w-32 h-32 bg-emerald-500/[0.04] rounded-tr-[8rem] -ml-16 -mb-16 font-Cairo"></div>
                
                <h4 class="font-black text-[var(--text-muted)] text-[13px] uppercase tracking-[0.3em] mb-12 px-6 font-Cairo text-start">📸 {{ __('صورة السند المالي') }}</h4>
                
                @if($userPackage->image_bond)
                    <div class="group relative font-Cairo">
                        <div class="w-full aspect-[3/4] rounded-[1.5rem] overflow-hidden bg-slate-900 border-8 border-[var(--glass-border)] shadow-[0_45px_90px_-20px_rgba(0,0,0,0.4)] transition-all duration-1000 group-hover:scale-[1.03] group-hover:rotate-2 font-Cairo shadow-emerald-500/5">
                            ...
                            <div class="absolute inset-0 bg-brand-primary/40 opacity-0 group-hover:opacity-100 transition-all duration-700 flex items-center justify-center backdrop-blur-md">
                                <a href="{{ asset('storage/' . $userPackage->image_bond) }}" target="_blank" class="px-10 py-5 bg-[var(--glass-bg)] text-brand-dark rounded-[1.2rem] text-[14px] font-black uppercase tracking-[0.2em] shadow-2xl transform translate-y-10 group-hover:translate-y-0 transition-all duration-700 font-Cairo">{{ __('عرض صورة السند') }}</a>
                            </div>
                        </div>
                        <div class="mt-8 flex items-center justify-center gap-3 text-start font-Cairo">
                            ...
                            <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('تم رفع الصورة بواسطة المستخدم.') }}</span>
                        </div>
                    </div>
                @else
                    <div class="w-full aspect-[3/4] rounded-[1.5rem] border-8 border-dashed border-[var(--glass-border)] flex flex-col items-center justify-center bg-[var(--main-bg)] gap-8 shadow-inner font-Cairo">
                        <div class="w-24 h-24 bg-[var(--glass-bg)]/50 dark:bg-[var(--glass-bg)]/5 rounded-3xl flex items-center justify-center shadow-inner font-Cairo">
                        ...
                        <div class="flex flex-col gap-2 font-Cairo">
                            <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-center font-Cairo">{{ __('لا يوجد مرفق') }}</span>
                            <span class="text-[14px] font-bold text-rose-500/60 font-Cairo text-center">{{ __('لا توجد صورة للسند مرفقة مع هذا الطلب.') }}</span>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="card-premium bg-[var(--glass-bg)] text-[var(--main-text)] border border-[var(--glass-border)] p-12 rounded-[2rem] shadow-xl relative overflow-hidden group text-start font-Cairo font-Cairo">
