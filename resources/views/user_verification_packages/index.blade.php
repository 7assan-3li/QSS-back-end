...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-start font-Cairo">
        <!-- Total Requests -->
        <div class="card-premium glass-panel p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group border border-[var(--glass-border)] text-start font-Cairo shadow-indigo-500/5">
            ...
            <div class="flex flex-col gap-2 relative z-10 text-start font-Cairo">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('إجمالي الطلبات') }}</span>
                <div class="flex items-baseline gap-3 text-start font-Cairo">
                    ...
                    <span class="text-[13px] font-black text-[var(--text-muted)] italic text-start font-Cairo">{{ __('طلب') }}</span>
                </div>
            </div>
            <div class="mt-6 flex items-center gap-2 text-[13px] font-black text-[var(--text-muted)] font-Cairo text-start">
        ...
        <!-- Pending Requests -->
        <div class="card-premium glass-panel p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group border border-[var(--glass-border)] text-start font-Cairo shadow-amber-500/5">
            ...
            <div class="flex flex-col gap-2 relative z-10 text-start font-Cairo">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('طلبات بانتظار المراجعة') }}</span>
                <div class="flex items-baseline gap-3 text-start font-Cairo">
                    ...
                    <span class="text-[13px] font-black text-amber-500 italic text-start font-Cairo">{{ __('بانتظار المراجعة') }}</span>
                </div>
            </div>
            <div class="mt-6 flex items-center gap-2 text-[13px] font-black text-amber-500 font-Cairo text-start">
        ...
        <!-- Approved Requests -->
        <div class="card-premium glass-panel p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group border border-[var(--glass-border)] text-start font-Cairo shadow-emerald-500/5">
            ...
            <div class="flex flex-col gap-2 relative z-10 text-start font-Cairo">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('طلبات مقبولة') }}</span>
                <div class="flex items-baseline gap-3 text-start font-Cairo">
                    ...
                    <span class="text-[13px] font-black text-emerald-500 italic text-start font-Cairo">{{ __('تمت الموافقة') }}</span>
                </div>
            </div>
        ...
        <!-- Rejected Requests -->
        <div class="card-premium glass-panel p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group border border-[var(--glass-border)] text-start font-Cairo shadow-rose-500/5">
            ...
            <div class="flex flex-col gap-2 relative z-10 text-start font-Cairo">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start">{{ __('طلبات مرفوضة') }}</span>
                <div class="flex items-baseline gap-3 text-start font-Cairo">
                    ...
                    <span class="text-[13px] font-black text-rose-500 italic text-start font-Cairo">{{ __('تم الرفض') }}</span>
                </div>
            </div>
            <div class="mt-6 flex items-center gap-2 text-[13px] font-black text-rose-500 font-Cairo text-start">
    ...
    <div class="card-premium glass-panel rounded-[2rem] shadow-2xl border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
        <div class="p-10 border-b border-[var(--glass-border)] flex flex-col md:flex-row justify-between items-center gap-8 text-start font-Cairo">
            <div class="text-start font-Cairo">
            ...
            <table class="w-full text-start border-collapse font-Cairo">
                <thead>
                    <tr class="bg-[var(--main-bg)] border-b border-[var(--glass-border)] text-start font-Cairo">
                        <th class="px-10 py-8 text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-start font-Cairo">{{ __('المستخدم') }}</th>
                        <th class="px-10 py-8 text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-start font-Cairo">{{ __('الباقة المطلوب') }}</th>
                        <th class="px-10 py-8 text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-start font-Cairo">{{ __('رقم السند') }}</th>
                        <th class="px-10 py-8 text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-center font-Cairo">{{ __('حالة الطلب') }}</th>
                        <th class="px-10 py-8 text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] text-start font-Cairo">{{ __('التاريخ') }}</th>
                ...
                <tbody class="divide-y divide-[var(--glass-border)] text-start font-Cairo">
                    @forelse ($userPackages as $request)
                        <tr class="hover:bg-brand-primary/[0.02] dark:hover:bg-brand-primary/[0.03] transition-all group text-start font-Cairo shadow-inner">
                            <td class="px-10 py-8 text-start font-Cairo">
                                <div class="flex items-center gap-5 text-start font-Cairo">
                                    ...
                                    <div class="flex flex-col text-start font-Cairo">
                                        ...
                                        <span class="text-[13px] font-bold text-[var(--text-muted)] font-mono tracking-wide text-start italic">{{ $request->user->email ?? '' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-8 text-start font-Cairo">
                            ...
                            <td class="px-10 py-8 text-start font-Cairo">
                                <span class="px-4 py-2 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-xl text-[13px] font-black font-mono tracking-[0.2em] border border-[var(--glass-border)] italic font-Cairo shadow-inner whitespace-nowrap inline-flex items-center justify-center">#{{ $request->number_bond }}</span>
                            </td>
                            <td class="px-10 py-8 text-center font-Cairo">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-5 py-2.5 rounded-2xl text-[12px] font-black uppercase tracking-[0.2em] {{ $statusColor }} border shadow-sm font-Cairo italic mx-auto whitespace-nowrap inline-flex items-center justify-center">
                                    {{ $statusLabel }}
                                </span>
                            </td>
                            <td class="px-10 py-8 text-start font-Cairo">
                                <span class="text-[13px] font-black text-[var(--text-muted)] font-mono tracking-[0.2em] italic text-start font-Cairo">{{ $request->created_at->format('Y-m-d') }}</span>
                            </td>
                            <td class="px-10 py-8 text-center font-Cairo">
                                <a href="{{ route('user-verification-packages.show', $request->id) }}" class="w-12 h-12 bg-[var(--main-bg)] text-[var(--text-muted)] hover:text-white hover:bg-brand-primary rounded-2xl transition-all shadow-sm border border-[var(--glass-border)] flex items-center justify-center mx-auto font-Cairo text-start" title="{{ __('عرض تفاصيل الطلب') }}">
                        ...
                        <tr>
                            <td colspan="6" class="px-10 py-40 text-center font-Cairo text-start">
                                <div class="flex flex-col items-center opacity-30 gap-8 font-Cairo text-start">
                                    <div class="w-24 h-24 bg-[var(--glass-border)] rounded-[3rem] flex items-center justify-center text-6xl shadow-inner animate-pulse font-Cairo italic">💎</div>
                                    <p class="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-start italic">{{ __('لا توجد طلبات اشتراك حالياً.') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
