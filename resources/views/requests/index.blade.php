...
@section('content')
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 font-Cairo text-start">
        ...
        <!-- Pending Orders -->
        <a href="{{ route('requests.index', ['status' => 'pending']) }}" class="card-premium glass-panel p-6 rounded-[2.5rem] shadow-xl border border-amber-100 dark:border-amber-500/20 hover:scale-[1.05] transition-all group text-center bg-amber-50/40 dark:bg-amber-950/20 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-3 relative z-10">
                ...
                <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] text-center font-Cairo">{{ __('قيد المعالجة') }}</span>
            </div>
        </a>

        <!-- Completed Orders -->
        <a href="{{ route('requests.index', ['status' => 'completed']) }}" class="card-premium glass-panel p-6 rounded-[2.5rem] shadow-xl border border-emerald-100 dark:border-emerald-500/20 hover:scale-[1.05] transition-all group text-center bg-emerald-50/40 dark:bg-emerald-950/20 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-3 relative z-10">
                ...
                <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] text-center font-Cairo">{{ __('عمليات مكتملة') }}</span>
            </div>
        </a>

        <!-- Unpaid Commissions -->
        <a href="{{ route('requests.index', ['commission' => 'unpaid']) }}" class="card-premium glass-panel p-6 rounded-[2.5rem] shadow-xl border border-rose-100 dark:border-rose-500/20 hover:scale-[1.05] transition-all group text-center bg-rose-50/40 dark:bg-rose-950/20 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-3 relative z-10">
                ...
                <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] text-center font-Cairo">{{ __('ديون معلقة') }}</span>
            </div>
        </a>

        <!-- Custom Services -->
        <a href="{{ route('requests.index', ['type' => 'custom']) }}" class="card-premium glass-panel p-6 rounded-[2.5rem] shadow-xl border border-blue-100 dark:border-blue-500/20 hover:scale-[1.05] transition-all group text-center bg-blue-50/40 dark:bg-blue-950/20 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-3 relative z-10">
                ...
                <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] text-center font-Cairo">{{ __('نماذج مخصصة') }}</span>
            </div>
        </a>

        <!-- Meetings -->
        <a href="{{ route('requests.index', ['type' => 'meeting']) }}" class="card-premium glass-panel p-6 rounded-[2.5rem] shadow-xl border border-violet-100 dark:border-violet-500/20 hover:scale-[1.05] transition-all group text-center bg-violet-50/40 dark:bg-violet-950/20 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-3 relative z-10">
                ...
                <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] text-center font-Cairo">{{ __('لقاءات مجدولة') }}</span>
            </div>
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-start">
        @forelse ($requests as $request)
            ...
            @endphp
            <div class="card-premium glass-panel p-10 rounded-[3.5rem] shadow-2xl border border-white/50 dark:border-slate-800/50 hover:shadow-brand-primary/10 transition-all duration-500 flex flex-col group relative overflow-hidden text-start font-Cairo">
                ...
                <div class="flex justify-between items-start mb-8 text-start">
                    ...
                    <span class="text-[10px] font-black text-slate-400 bg-slate-50 dark:bg-slate-900 px-4 py-1.5 rounded-xl border border-slate-100 dark:border-slate-800 font-mono text-start">ID: #{{ $request->id }}</span>
                </div>
                        ...
                        @foreach ($request->services as $service)
                             <div class="px-3.5 py-2 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-3 transition-all hover:bg-white dark:hover:bg-slate-800 text-start">
                                ...
                                @if($service->pivot->is_main)
                                    <span class="text-[7px] font-black bg-indigo-500 text-white px-2 py-0.5 rounded-md uppercase tracking-tighter shadow-sm font-Cairo text-start">{{ __('أساسية') }}</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <!-- Financial Details -->
                <div class="grid grid-cols-2 gap-6 pt-10 border-t border-slate-100 dark:border-slate-800/50 mb-10 text-start">
                    ...
                    <div class="flex flex-col gap-1.5 text-end text-start">
                        <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] font-Cairo text-start">{{ __('عمولة المنصة (10%)') }}</span>
