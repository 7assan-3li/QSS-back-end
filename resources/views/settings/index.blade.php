...
<div class="max-w-7xl mx-auto space-y-16 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-10 text-start font-Cairo">
        ...
        <div class="flex items-center gap-5 px-10 py-5 bg-emerald-500/5 rounded-[2rem] border border-emerald-500/20 text-emerald-600 text-[13px] font-black uppercase tracking-[0.2em] font-Cairo shadow-sm">
    ...
    @if(session('success'))
        <div class="card-premium glass-panel bg-emerald-500/5 border-emerald-500/20 p-10 rounded-[3.5rem] flex items-center gap-8 animate-fade-in shadow-2xl shadow-emerald-500/10 text-start font-Cairo">
            ...
            <div class="text-start font-Cairo">
                ...
                <p class="text-[14px] font-bold text-emerald-600 font-Cairo text-start italic opacity-80">{{ session('success') }}</p>
            </div>
        </div>
    @endif
                ...
                <div class="{{ $isPolicy ? 'lg:col-span-3' : '' }} card-premium glass-panel p-12 rounded-[4.5rem] shadow-2xl relative border border-[var(--glass-border)] flex flex-col group hover:-translate-y-4 transition-all duration-1000 text-start overflow-hidden font-Cairo shadow-indigo-500/5">
                    ...
                    <div class="flex items-start justify-between mb-12 text-start font-Cairo">
                        <div class="text-start font-Cairo">
                            ...
                            <span class="px-4 py-1.5 bg-slate-900 text-white rounded-xl text-[14px] font-black font-mono tracking-[0.3em] uppercase shadow-lg shadow-slate-950/20 italic whitespace-nowrap inline-flex items-center justify-center">
                    ...
                    <div class="relative mt-auto text-start font-Cairo">
                        @if($isPolicy)
                            <div class="absolute top-6 right-8 pointer-events-none font-Cairo opacity-40">
                                <span class="text-[12px] font-black font-Cairo tracking-[0.3em] uppercase italic">{{ __('محتوى السياسة') }}</span>
                            </div>
                            <textarea id="{{ $setting->key }}" name="{{ $setting->key }}" required rows="8" class="w-full bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[2.5rem] px-10 py-10 text-sm font-bold focus:border-brand-primary focus:ring-[20px] focus:ring-brand-primary/5 transition-all outline-none font-Cairo italic leading-relaxed shadow-inner resize-none">{{ $setting->value }}</textarea>
                        @else
                            <div class="absolute inset-y-0 right-0 flex items-center pr-10 pointer-events-none font-Cairo">
                                <span class="text-[13px] font-black font-Cairo tracking-[0.4em] uppercase italic opacity-60">{{ __('القيمة') }}</span>
                            </div>
