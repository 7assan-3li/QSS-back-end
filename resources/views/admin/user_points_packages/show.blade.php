    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            <div class="flex items-center gap-5 mb-5 text-start font-Cairo">
                <a href="{{ route('admin.user-points-packages.index') }}" class="w-14 h-14 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-2xl flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-sm border border-[var(--glass-border)] font-Cairo">
        ...
        <div class="flex items-center gap-4 text-start font-Cairo">
            <span class="px-8 py-3 rounded-[2rem] text-[13px] font-black uppercase tracking-[0.2em] font-Cairo text-start shadow-xl border {{ $statusColor }} font-Cairo">
                {{ $statusLabel }}
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 text-start font-Cairo">
            ...
            <div class="card-premium glass-panel p-14 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-14 text-start font-Cairo">
                    ...
                    <div class="card-premium glass-panel p-8 rounded-[1.5rem] border border-[var(--glass-border)] flex items-center gap-6 group hover:scale-[1.03] transition-all shadow-sm text-start font-Cairo">
                        <div class="w-16 h-16 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-[1.2rem] flex items-center justify-center text-2xl shadow-inner group-hover:rotate-6 transition-transform font-Cairo italic">🏦</div>
                        <div class="flex flex-col text-start font-Cairo">
                            <span class="text-[12px] font-black uppercase tracking-[0.3em] mb-2 font-Cairo text-start opacity-60">{{ __('البنك المُحول منه') }}</span>
                            <span class="text-sm font-black font-Cairo text-start italic">{{ $subscription->bank_name }}</span>
                        </div>
                    </div>

                    <!-- Transfer Number -->
                    <div class="card-premium glass-panel p-8 rounded-[1.5rem] border border-[var(--glass-border)] flex items-center gap-6 group hover:scale-[1.03] transition-all shadow-sm text-start font-Cairo shadow-amber-500/5">
                        ...
                        <div class="flex flex-col text-start font-Cairo">
                            <span class="text-[12px] font-black uppercase tracking-[0.3em] mb-2 font-Cairo text-start opacity-60">{{ __('رقم الحوالة / السند') }}</span>
                ...
                <div class="space-y-6 mb-14 text-start font-Cairo">
                    ...
                    <div class="relative group rounded-[2rem] overflow-hidden border-8 border-[var(--glass-border)] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] bg-[var(--glass-border)] font-Cairo">
                         <a href="{{ asset('storage/' . $subscription->bond_image) }}" target="_blank" class="block font-Cairo text-start">
                             ...
                             <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-all duration-700 flex flex-col items-center justify-center p-12 backdrop-blur-md">
                                 <div class="w-20 h-20 bg-[var(--glass-bg)]/20 rounded-full flex items-center justify-center mb-6 border border-white/30 animate-pulse font-Cairo">
                        ...
                        <div class="flex flex-col sm:flex-row gap-8 text-start font-Cairo">
                            <form id="approve-points-subscription-{{ $subscription->id }}" action="{{ route('admin.user-points-packages.approve', $subscription->id) }}" method="POST" class="flex-[3] text-start">
                                ...
                                <button type="button" 
                                    ...
                                    })" class="w-full py-7 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-[1.5rem] text-[14px] font-black uppercase tracking-[0.3em] shadow-[0_20px_40px_-10px_rgba(16,185,129,0.4)] hover:scale-[1.03] transition-all duration-500 font-Cairo flex items-center justify-center gap-5 text-start">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                    {{ __('موافقة وتفعيل الشحن') }}
                                </button>
                            </form>
                            <button type="button" class="flex-1 px-10 py-7 bg-rose-500/5 text-rose-600 border border-rose-500/20 rounded-[1.5rem] text-[14px] font-black uppercase tracking-[0.3em] hover:bg-rose-600 hover:text-white transition-all duration-500 font-Cairo flex items-center justify-center gap-3 text-start" onclick="document.getElementById('reject-form').classList.toggle('hidden')">
                        ...
                        <div id="reject-form" class="mt-10 p-12 bg-rose-50 dark:bg-rose-950/10 border-2 border-rose-500/20 rounded-[2rem] hidden animate-fade-in text-start font-Cairo">
                            <form action="{{ route('admin.user-points-packages.reject', $subscription->id) }}" method="POST" class="space-y-8 text-start font-Cairo">
                                ...
                                <div class="space-y-4 text-start font-Cairo">
                                    ...
                                    <textarea name="admin_note" rows="4" required placeholder="{{ __('مثال: يرجى إرفاق نسخة أكثر وضوحاً من السند البنكي، أو التحقق من تطابق رقم العملية...') }}" class="w-full px-10 py-8 bg-[var(--glass-bg)] border-2 border-[var(--glass-border)] rounded-[1.5rem] text-sm font-black outline-none focus:border-rose-500 focus:ring-[20px] focus:ring-rose-500/5 transition-all text-[var(--main-text)] font-Cairo shadow-inner italic text-start"></textarea>
                                </div>
                                <div class="flex gap-6 text-start font-Cairo">
                                    <button type="submit" class="flex-1 bg-rose-600 text-white py-6 rounded-[1.2rem] text-[14px] font-black uppercase tracking-[0.3em] shadow-xl shadow-rose-600/20 hover:scale-[1.02] transition-all font-Cairo text-start font-Cairo">{{ __('تأكيد الرفض النهائي') }}</button>
                                    <button type="button" class="px-12 py-6 bg-slate-200 text-[var(--text-muted)] rounded-[1.2rem] text-[14px] font-black uppercase tracking-[0.2em] font-Cairo text-start" onclick="document.getElementById('reject-form').classList.add('hidden')">{{ __('إلغاء') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                @else
                    ...
                    <div class="mt-14 p-12 bg-[var(--main-bg)] rounded-[2rem] border border-[var(--glass-border)] shadow-inner text-start font-Cairo">
                        <div class="flex flex-col md:flex-row items-center justify-between gap-10 mb-12 text-start font-Cairo">
                            <div class="flex items-center gap-5 text-start font-Cairo">
                                <span class="text-[13px] font-black uppercase tracking-[0.3em] font-Cairo text-start opacity-60">{{ __('حالة الطلب') }}</span>
                                @if($subscription->status == 'approved')
                                    <span class="px-8 py-3 bg-emerald-500/10 text-emerald-600 rounded-[2rem] text-[14px] font-black border border-emerald-500/20 shadow-xl shadow-emerald-500/10 font-Cairo italic text-start">{{ __('تم القبول واعتماد الشحن') }} ✓</span>
                                @else
                                    <span class="px-8 py-3 bg-rose-500/10 text-rose-600 rounded-[2rem] text-[14px] font-black border border-rose-500/20 shadow-xl shadow-rose-500/5 font-Cairo italic text-start">{{ __('رفض الطلب') }} ✋</span>
                                @endif
                            </div>
                            <div class="flex items-center gap-4 text-start font-Cairo">
                                <span class="text-[12px] font-black font-Cairo text-start italic opacity-60">{{ __('المسؤول المراجع') }}:</span>
                        ...
                        @if($subscription->admin_note)
                            <div class="space-y-4 bg-[var(--glass-bg)] p-10 rounded-[1.5rem] border border-[var(--glass-border)] shadow-sm text-start font-Cairo italic">
                                <span class="text-[12px] font-black uppercase tracking-[0.3em] block font-Cairo text-start font-Cairo opacity-60">{{ __('ملاحظات الإدارة') }}:</span>
        ...
        <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
            ...
            <div class="card-premium glass-panel p-10 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
                ...
                <h4 class="font-black text-[13px] uppercase tracking-[0.3em] mb-10 px-4 font-Cairo text-start opacity-60">👤 {{ __('بيانات صاحب الطلب') }}</h4>
                <div class="flex items-center gap-6 text-start font-Cairo">
                    ...
                    <div class="flex flex-col text-start font-Cairo">
                        ...
                        <span class="text-[13px] font-bold font-mono mt-2 text-start italic opacity-60">{{ $subscription->user->email ?? __('لا يوجد بريد') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="card-premium glass-panel p-10 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
                ...
                <h4 class="font-black text-[13px] uppercase tracking-[0.3em] mb-10 px-4 font-Cairo text-start font-Cairo opacity-60">💎 {{ __('تفاصيل باقة النقاط') }}</h4>
                ...
                <div class="p-12 bg-slate-950 dark:bg-[var(--glass-bg)] rounded-[2rem] text-center mb-10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4)] transform hover:scale-[1.05] transition-all duration-700 text-start group font-Cairo">
                    <p class="text-6xl font-black text-brand-primary leading-none mb-4 font-mono group-hover:rotate-3 transition-transform text-center font-Cairo">{{ number_format(($subscription->package->points ?? 0) + ($subscription->package->bonus_points ?? 0)) }}</p>
                    <p class="text-[13px] font-black text-white/40 dark:text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo text-center font-Cairo opacity-40">{{ __('إجمالي النقاط المضافة') }}</p>
                </div>

                <div class="flex justify-between items-center px-6 text-start font-Cairo">
                    <span class="text-[13px] font-black uppercase tracking-[0.3em] font-Cairo text-start opacity-60">{{ __('المبلغ المدفوع') }}:</span>
                    <div class="text-start font-Cairo">
                        ...
                        <span class="text-xs font-black font-Cairo mr-2 tracking-widest text-start opacity-60">{{ __('ريال يمني') }}</span>
                    </div>
                </div>
            </div>
        </div>
