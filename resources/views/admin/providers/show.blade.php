...
<div class="max-w-7xl mx-auto space-y-8 mt-4 animate-fade-in text-start font-Cairo" x-data="{ tab: 'overview' }">
    ...
    <div class="glass-panel p-10 rounded-[3.5rem] relative overflow-hidden group shadow-2xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 text-start">
        ...
        <div class="flex flex-col items-end gap-3 w-full lg:w-fit relative z-10 text-start">
             <div class="flex items-center gap-3 w-full">
                <a href="{{ route('users.edit', $provider->id) }}" class="flex-1 lg:flex-none px-8 py-3 bg-brand-primary text-white text-[11px] font-black rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-brand-primary/20 flex items-center gap-2 justify-center">
            ...
            @if(!($provider->provider_verified_until && \Carbon\Carbon::parse($provider->provider_verified_until)->isFuture()))
            <span class="text-[10px] font-black text-amber-600 bg-amber-500/5 px-4 py-2 rounded-xl border border-amber-500/10 italic text-start flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-start">
        <div class="card-premium glass-panel p-8 text-start">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest font-Cairo">{{ __('إجمالي الطلبات') }}</span>
            ...
            <div class="flex items-baseline gap-2 mt-4">
                ...
                <span class="text-[10px] font-black text-indigo-500">{{ __('طلب مستلم') }}</span>
            </div>
        </div>
        <div class="card-premium glass-panel p-8 text-start">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest font-Cairo">{{ __('العمولات المحققة للمنصة') }}</span>
            ...
            <div class="flex items-baseline gap-2 mt-4">
                ...
                <span class="text-[10px] font-black text-amber-500 opacity-60">YER</span>
            </div>
        </div>
        <div class="card-premium glass-panel p-8 text-start">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest font-Cairo">{{ __('متوسط التقييم') }}</span>
            ...
            <div class="flex items-baseline gap-2 mt-4">
                ...
                <span class="text-[10px] font-black text-emerald-500 opacity-60 uppercase">{{ __('نجمة') }}</span>
            </div>
        </div>
        <div class="card-premium glass-panel p-8 text-start">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest font-Cairo">{{ __('إجمالي إيرادات المزود') }}</span>
            ...
            <div class="flex items-baseline gap-2 mt-4">
                ...
                <span class="text-[10px] font-black text-brand-primary opacity-60">YER</span>
            </div>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-[3.5rem] shadow-2xl overflow-hidden font-Cairo text-start min-h-[700px] flex flex-col">
        <div class="flex items-center gap-10 px-10 border-b border-slate-100 dark:border-white/5 bg-slate-50/20 dark:bg-slate-900/40 overflow-x-auto no-scrollbar">
            <button @click="tab = 'overview'" :class="tab === 'overview' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('نظرة عامة') }}</button>
            <button @click="tab = 'services'" :class="tab === 'services' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('دليل الخدمات') }}</button>
            <button @click="tab = 'portfolio'" :class="tab === 'portfolio' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('معرض الأعمال') }}</button>
            <button @click="tab = 'requests'" :class="tab === 'requests' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('سجل الطلبات') }}</button>
            <button @click="tab = 'financials'" :class="tab === 'financials' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('البيانات المالية') }}</button>
            <button @click="tab = 'reviews'" :class="tab === 'reviews' ? 'text-brand-primary border-b-4 border-brand-primary' : 'text-slate-400 border-b-4 border-transparent'" class="py-8 text-xs font-black transition-all whitespace-nowrap">{{ __('التقييمات') }}</button>
        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div class="space-y-8">
                        ...
                        <div class="grid grid-cols-2 gap-8">
                            <div class="flex flex-col gap-2 p-6 bg-slate-50 dark:bg-white/5 rounded-3xl">
                                <span class="text-[9px] font-black text-slate-400 uppercase">{{ __('المسمى الوظيفي') }}</span>
                                <span class="text-[12px] font-black">{{ $provider->profile->job_title ?? '---' }}</span>
                            </div>
                            <div class="flex flex-col gap-2 p-6 bg-slate-50 dark:bg-white/5 rounded-3xl">
                                <span class="text-[9px] font-black text-slate-400 uppercase">{{ __('رقم الهاتف') }}</span>
                                <span class="text-[12px] font-black font-mono tabular-nums leading-none">
                                    {{ $provider->profile->profilePhones->first()->phone ?? '---' }}
                                </span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-3 p-6 bg-slate-50 dark:bg-white/5 rounded-3xl">
                             <span class="text-[9px] font-black text-slate-400 uppercase">{{ __('نبذة عن المزود (Bio)') }}</span>
                             <p class="text-[11px] font-black leading-relaxed text-slate-600 dark:text-slate-300">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center justify-between p-6 border border-slate-100 dark:border-white/5 rounded-3xl hover:bg-slate-50 transition-all">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-[11px] font-black">{{ $pkg->name }}</span>
                                        <span class="text-[9px] font-black text-slate-400 uppercase">{{ __('تاريخ الاشتراك: ') }} {{ $pkg->pivot->created_at->format('Y-m-d') }}</span>
                                    </div>
                                </div>
                                <span class="px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider {{ $pkg->pivot->status === 'approved' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-100 text-slate-500' }}">
                                    {{ $pkg->pivot->status }}
                                </span>
                            </div>
                            @empty
                            <div class="p-10 text-center border-2 border-dashed border-slate-100 dark:border-white/5 rounded-[2.5rem] flex flex-col items-center gap-4">
                                ...
                                <span class="text-[10px] font-black text-slate-400 italic">{{ __('المزود لم يشترك في حزم توثيق بعد.') }}</span>
                            </div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div class="card-premium glass-panel p-8 text-start group hover:border-brand-primary/50 transition-all relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-start relative z-10 w-full">
                            ...
                            <span class="px-4 py-2 bg-slate-100 dark:bg-white/5 rounded-xl text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $service->category->name ?? __('عام') }}</span>
                        </div>
                        ...
                        <div class="mt-8 flex items-baseline gap-2">
                             ...
                             <span class="text-[10px] font-black opacity-30 tracking-tighter">YER</span>
                        </div>
                        
                        <div class="mt-8 pt-8 border-t border-slate-100 dark:border-white/5 flex flex-col gap-2">
                            <div class="flex justify-between items-center">
                                <span class="text-[10px] font-black text-slate-400">{{ __('عمولة النظام:') }}</span>
                                <span class="text-[9px] font-black text-slate-400">{{ $provider->commission ?? '10' }}%</span>
                            </div>
                            <a href="{{ route('services.show', $service->id) }}" class="mt-2 text-brand-primary text-[10px] font-black hover:translate-x-2 transition-transform flex items-center gap-2">
                    ...
                    @empty
                        <div class="col-span-3 py-40 text-center flex flex-col items-center gap-6">
                            <div class="w-24 h-24 bg-slate-50 dark:bg-white/5 rounded-full flex items-center justify-center text-slate-200">
                            ...
                            <p class="text-slate-400 font-black italic text-sm">{{ __('لا توجد خدمات مضافة لهذا الشريك حالياً.') }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div x-show="tab === 'portfolio'" class="animate-fade-in space-y-12">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                    ...
                    <div class="card-premium glass-panel p-6 flex flex-col gap-6 text-start group hover:scale-[1.02] transition-all">
                        <div class="relative overflow-hidden rounded-[2.5rem] shadow-inner aspect-[4/3] bg-slate-100 dark:bg-white/5">
                            @if($work->image)
                        ...
                        <div class="flex flex-col gap-2">
                             ...
                             <p class="text-[10px] font-black text-slate-400 line-clamp-2 leading-relaxed">{{ $work->description ?? __('لا يوجد وصف مضاف...') }}</p>
                        </div>
                    </div>
                    @empty
                    <div class="col-span-3 py-40 text-center flex flex-col items-center gap-6">
                        ...
                        <p class="text-slate-400 font-black italic text-sm">{{ __('لم يقم هذا المزود برفع أي أعمال سابقة حتى الآن.') }}</p>
                    </div>
                    @endforelse
                </div>
            </div>
                    ...
                    <table class="w-full text-start">
                        <thead>
                            <tr class="bg-slate-50/40 dark:bg-slate-900/40 border-b border-slate-100 dark:border-white/5">
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest">{{ __('العميل والطلب') }}</th>
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest">{{ __('الخدمة الرئيسية') }}</th>
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest text-center">{{ __('الحالة') }}</th>
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest">{{ __('التكلفة الإجمالية') }}</th>
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest">{{ __('العمولة (المنصة)') }}</th>
                                <th class="table-header-cell px-10 py-8 text-[10px] uppercase tracking-widest text-center">{{ __('الإجراء') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-white/3">
                            ...
                            <tr class="hover:bg-brand-primary/[0.01] transition-all group">
                                <td class="px-10 py-8">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-[11px] font-black group-hover:text-brand-primary transition-colors">{{ $req->user->name ?? '---' }}</span>
                                        <div class="flex items-center gap-2">
                                            <span class="text-[9px] font-black text-slate-400 font-mono tracking-tighter uppercase tabular-nums">ID: #{{ $req->id }}</span>
                                            <span class="w-1 h-1 bg-slate-200 rounded-full"></span>
                                            <span class="text-[9px] font-black text-slate-400 font-mono uppercase">{{ $req->created_at->format('Y/m/d') }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-10 py-8">
                                    <span class="text-[11px] font-black text-slate-600 dark:text-slate-300 leading-tight block max-w-[150px] truncate">
                                ...
                                <td class="px-10 py-8 text-center">
                                    @php
                                        ...
                                        $config = $statusConfig[$req->status] ?? ['bg' => 'bg-slate-100', 'text' => 'text-slate-500'];
                                    @endphp
                                    <span class="px-5 py-2 {{ $config['bg'] }} {{ $config['text'] }} rounded-2xl text-[9px] font-black uppercase tracking-wider inline-block">
                                        {{ $req->status }}
                                    </span>
                                </td>
                                <td class="px-10 py-8 text-[12px] font-black font-mono tabular-nums leading-none">
                                    {{ number_format($req->total_price, 0) }} <span class="text-[9px] opacity-30 italic">YER</span>
                                </td>
                                <td class="px-10 py-8 text-[12px] font-black text-amber-500 font-mono italic tabular-nums leading-none">
                                    {{ number_format($req->commission_amount, 0) }} <span class="text-[9px] opacity-40">YER</span>
                                </td>
                                <td class="px-10 py-8 text-center">
                                    <a href="{{ route('requests.show', $req->id) }}" class="p-3 bg-slate-50 dark:bg-white/5 text-slate-400 rounded-2xl hover:bg-brand-primary hover:text-white transition-all inline-flex items-center">
                            ...
                            <tr><td colspan="6" class="py-40 text-center flex flex-col items-center gap-6">
                                <p class="text-slate-400 font-black italic text-sm">{{ __('لم يتم تنفيذ أي طلبات بعد.') }}</p>
                            </td></tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div x-show="tab === 'financials'" class="animate-fade-in text-start space-y-12">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div class="card-premium glass-panel p-12 flex flex-col gap-8 text-start relative overflow-hidden">
                        ...
                        <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 relative z-10">
                            <span class="w-5 h-5 bg-indigo-500/10 text-indigo-500 rounded-lg flex items-center justify-center">💳</span>
                            {{ __('إعدادات العمولات') }}
                        </h4>
                        <div class="space-y-6 relative z-10">
                            <div class="flex items-center justify-between p-6 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/5">
                                <span class="text-[11px] font-black">{{ __('هل العميل مستثنى من العمولات؟') }}</span>
                                <span class="px-5 py-2 {{ $provider->no_commission ? 'bg-rose-500/10 text-rose-600' : 'bg-emerald-500/10 text-emerald-600' }} text-[9px] font-black rounded-xl border border-transparent shadow-sm">
                                    {{ $provider->no_commission ? __('نعم (مستثنى)') : __('لا (يتم الخصم)') }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between p-6 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/5">
                                <span class="text-[11px] font-black">{{ __('نسبة العمولة الخاصة بهذا المزود') }}</span>
                    ...
                    <div class="card-premium glass-panel p-12 flex flex-col gap-8 text-start">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="p-8 bg-slate-50 dark:bg-white/5 rounded-3xl flex flex-col gap-4 text-start border border-slate-100 dark:border-white/5 relative group hover:border-brand-primary/20 transition-all">
                                 <div class="flex justify-between items-center text-start">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-white dark:bg-slate-800 rounded-xl shadow-sm flex items-center justify-center text-xl">🏛️</div>
                                        <span class="text-[12px] font-black text-brand-primary leading-none">{{ $bank->name }}</span>
                                    </div>
                                    <span class="text-[9px] font-black opacity-40 uppercase tracking-tighter">{{ __('رصيد نشط') }}</span>
                                 </div>
                                 <div class="mt-2 flex items-center justify-between">
                                     <span class="text-[14px] font-black font-mono tracking-widest tabular-nums text-slate-600 dark:text-slate-200">
                            ...
                            @empty
                                <div class="py-16 text-center border border-dashed border-slate-200 dark:border-white/5 rounded-3xl">
                                    <span class="text-[11px] font-black text-slate-400 italic">{{ __('لم يقم المزود بإضافة أي حسابات بنكية حتى الآن.') }}</span>
                                </div>
                            @endforelse
                        </div>
                    </div>
            ...
            <div x-show="tab === 'reviews'" class="animate-fade-in text-start space-y-12">
                <div class="flex items-center justify-between mb-8">
                     ...
                     <div class="flex items-center gap-2 px-6 py-3 bg-emerald-500/5 text-emerald-600 rounded-2xl border border-emerald-500/10">
                         <span class="text-[11px] font-black">{{ __('التقييم الإجمالي:') }}</span>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="card-premium glass-panel p-10 flex flex-col gap-6 text-start relative group">
                        <div class="flex justify-between items-start">
                             <div class="flex items-center gap-4">
                                 <div class="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-xl font-black text-slate-400">{{ mb_substr($reqReviewed->user->name ?? '?', 0, 1) }}</div>
                                 <div class="flex flex-col">
                                     <span class="text-[11px] font-black">{{ $reqReviewed->user->name ?? '---' }}</span>
                                     <span class="text-[9px] font-black text-slate-400 uppercase">{{ __('طلب رقم #') }}{{ $reqReviewed->id }}</span>
                                 </div>
                             </div>
                        ...
                        <p class="text-[11px] font-black leading-relaxed text-slate-600 dark:text-slate-300 italic">
                            "{{ $reqReviewed->review->comment ?? __('لا يوجد تعليق نصي...') }}"
                        </p>
                        <span class="text-[9px] font-black text-slate-400 uppercase text-start opacity-40 group-hover:opacity-100 transition-opacity">
                            {{ $reqReviewed->review->created_at->diffForHumans() }}
                        </span>
                    </div>
                    @empty
                    <div class="col-span-2 py-32 text-center flex flex-col items-center gap-6">
                        <div class="w-20 h-20 bg-slate-50 dark:bg-white/5 rounded-full flex items-center justify-center text-slate-200">
                        ...
                        <p class="text-slate-400 font-black italic text-sm">{{ __('لا توجد تقييمات مسجلة لهذا المزود حتى الآن.') }}</p>
                    </div>
                    @endforelse
                </div>
            </div>
...
<style>
    ...
    .card-premium {
        @apply transition-all duration-500 border border-slate-100 dark:border-white/5 hover:shadow-2xl hover:shadow-brand-primary/10;
    }
