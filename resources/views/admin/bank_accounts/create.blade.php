...
@section('title', __('إضافة قناة دفع'))
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div class="space-y-4">
            ...
            <h3 class="text-4xl font-black text-[var(--main-text)] tracking-tight">
                {{ __('تجهيز') }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">{{ __('قناة دفع جديدة') }}</span>
            </h3>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.bank-accounts.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-16 relative z-10">
        @csrf
            ...
            @if ($errors->any())
                <div class="glass-panel bg-rose-500/5 border-rose-500/20 p-8 rounded-[3rem] animate-shake">
                    <div class="flex items-center gap-4 mb-4 text-rose-600">
                        ...
                        <h5 class="text-xs font-black uppercase tracking-widest italic">{{ __('يرجى مراجعة المدخلات:') }}</h5>
                    </div>
                ...
                <div class="space-y-12">
                    <div class="space-y-6">
                        ...
                        <p class="text-[10px] font-bold text-[var(--text-muted)] text-center italic leading-relaxed px-4">
                            {{ __('عند التفعيل، ستظهر هذه القناة فوراً في قائمة خيارات الدفع للمستخدمين.') }}
                        </p>
                    </div>

                    <div class="space-y-6 pt-12 border-t border-[var(--glass-border)]">
                        <button type="submit" class="btn-action-primary w-full py-8 !rounded-3xl shadow-2xl hover:!scale-[1.02] active:!scale-95 transition-all duration-500 !text-[12px]">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                            {{ __('اعتماد القناة المالية') }}
                        </button>
