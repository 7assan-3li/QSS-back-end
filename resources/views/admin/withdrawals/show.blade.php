...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        ...
        <div class="flex items-center gap-4 text-start font-Cairo">
            <span class="px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] font-Cairo text-start shadow-xl
                @if($withdrawal->status == 'pending') bg-amber-500/10 text-amber-600 border border-amber-500/20 shadow-amber-500/5
                @elseif($withdrawal->status == 'approved') bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 shadow-emerald-500/5
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 text-start font-Cairo">
                        ...
                        <form action="{{ route('admin.withdrawals.approve', $withdrawal->id) }}" method="POST" enctype="multipart/form-data" class="space-y-12 text-start font-Cairo">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 text-start font-Cairo">
                                <div class="space-y-4 text-start font-Cairo">
                                    ...
                                    <input type="text" name="bond_number" required placeholder="REFERENCE_CODE_00X"
                                           class="w-full px-10 py-6 bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800/80 rounded-[2rem] text-sm font-black outline-none focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all dark:text-white font-mono tracking-widest text-start italic shadow-inner">
                                </div>
                                ...
                                <div class="space-y-4 text-start font-Cairo">
                                    ...
                                    <div class="relative group text-start font-Cairo">
                                        <input type="file" name="bond_image" required accept="image/*" 
                                               class="w-full px-10 py-6 bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800/80 rounded-[2rem] text-[10px] font-black outline-none cursor-pointer file:hidden text-slate-400 focus:border-emerald-500 transition-all font-Cairo text-start italic shadow-inner">
                            ...
                            <div class="pt-8 flex flex-col sm:flex-row gap-8 text-start font-Cairo">
                                <button type="button" 
                                    ...
                                    class="flex-[3] px-12 py-7 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-[0_25px_50px_-15px_rgba(16,185,129,0.4)] hover:scale-[1.03] active:scale-95 transition-all duration-500 font-Cairo flex items-center justify-center gap-5 text-start font-Cairo">
                                ...
                                <button type="button" class="flex-1 px-10 py-7 bg-rose-500/5 text-rose-600 border border-rose-500/20 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] hover:bg-rose-600 hover:text-white transition-all duration-500 font-Cairo flex items-center justify-center gap-4 text-start shadow-sm font-Cairo" onclick="document.getElementById('reject-form').classList.toggle('hidden')">
                        ...
                        <div id="reject-form" class="mt-12 p-12 bg-rose-50 dark:bg-rose-950/10 border-2 border-rose-500/20 rounded-[4rem] hidden animate-fade-in text-start font-Cairo">
                            <form id="reject-withdrawal-{{ $withdrawal->id }}" action="{{ route('admin.withdrawals.reject', $withdrawal->id) }}" method="POST" class="space-y-8 text-start font-Cairo">
                                ...
                                <div class="space-y-4 text-start font-Cairo">
                                    ...
                                    <textarea name="admin_note" rows="4" required placeholder="{{ __('توضيح النواقص أو أسباب الرفض الفني...') }}" 
                                              class="w-full px-10 py-8 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[2.5rem] text-sm font-black outline-none focus:border-rose-500 focus:ring-[20px] focus:ring-rose-500/5 transition-all dark:text-white font-Cairo shadow-inner italic text-start"></textarea>
                                </div>
                                <div class="flex gap-6 text-start font-Cairo">
                                    <button type="button" 
                                        ...
                                        class="flex-1 bg-rose-600 text-white py-6 rounded-[2rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-xl shadow-rose-600/20 hover:scale-[1.02] transition-all font-Cairo text-start">{{ __('تجميد المطالبة نهائياً') }}</button>
                                    <button type="button" class="px-12 py-6 bg-slate-200 dark:bg-slate-800 text-slate-500 rounded-[2rem] text-[11px] font-black uppercase tracking-[0.2em] font-Cairo text-start" onclick="document.getElementById('reject-form').classList.add('hidden')">{{ __('إلغاء المعارضة') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="mt-14 p-14 bg-slate-50 dark:bg-slate-950/60 rounded-[4.5rem] border border-slate-100 dark:border-slate-800/80 shadow-inner text-start font-Cairo">
                        <div class="flex flex-col md:flex-row items-center justify-between gap-10 mb-14 text-start font-Cairo">
                            <div class="flex items-center gap-6 text-start font-Cairo">
                                ...
                                @else
                                    <span class="px-10 py-4 bg-rose-500/10 text-rose-600 rounded-[2.5rem] text-[11px] font-black border border-rose-500/20 shadow-xl shadow-rose-500/5 font-Cairo italic text-start">{{ __('المطالبة مجمدة إدارياً') }} 🚫</span>
                                @endif
                            </div>
                            <div class="flex items-center gap-4 text-start font-Cairo">
                                    ...
                                    <div class="relative rounded-[3rem] overflow-hidden border-8 border-white dark:border-slate-900 shadow-2xl text-start font-Cairo bg-slate-950">
                                        <a href="{{ asset('storage/' . $withdrawal->bond_image) }}" target="_blank" class="block text-start font-Cairo">
                                            ...
                                            <div class="absolute inset-0 bg-emerald-600/30 opacity-0 group-hover:opacity-100 transition-all duration-700 backdrop-blur-sm flex items-center justify-center font-Cairo">
                                                <div class="bg-white text-slate-900 px-8 py-4 rounded-2xl text-[11px] font-black shadow-3xl font-Cairo rotate-3">{{ __('تحليل السند') }} 🔎</div>
                                            </div>
                                        </a>
                                    </div>
                                </div>
        ...
        <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
            ...
            <div class="card-premium bg-gradient-to-br from-amber-500 to-amber-600 text-white p-12 rounded-[4.5rem] shadow-[0_40px_80px_-20px_rgba(245,158,11,0.4)] relative overflow-hidden group text-start font-Cairo">
                <div class="absolute -top-12 -right-12 w-48 h-48 bg-white/10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000 font-Cairo"></div>
                <div class="relative z-10 font-Cairo">
                    <h4 class="font-black text-white text-[11px] uppercase tracking-[0.4em] mb-6 flex items-center gap-4 font-Cairo text-start">
                        <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center font-Cairo">
                    ...
                    <p class="text-[11px] leading-[2] font-black text-white/95 font-Cairo text-start italic">
