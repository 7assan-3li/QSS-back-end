...
<div class="space-y-8 mt-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
        <div class="card-premium glass-panel p-6 relative overflow-hidden group">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[13px] font-black inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">
        ...
        <div class="card-premium glass-panel p-6 relative overflow-hidden group">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[13px] font-black inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">
        ...
        <div class="card-premium glass-panel p-6 relative overflow-hidden group">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[13px] font-black inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">
        ...
        <div class="card-premium bg-gradient-to-br from-brand-primary to-brand-primary-hover p-6 relative overflow-hidden group shadow-xl shadow-brand-primary/20 border-0">
            <div class="absolute -top-10 -right-10 w-40 h-40 bg-[var(--glass-bg)]/10 rounded-full blur-3xl transition-all group-hover:scale-125"></div>
            <div class="relative z-10 text-white">
                <div class="w-12 h-12 rounded-2xl bg-[var(--glass-bg)]/20 flex items-center justify-center mb-4 backdrop-blur-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in font-Cairo">
        <a href="{{ route('requests.index') }}" class="glass-panel p-6 rounded-3xl border border-[var(--glass-border)] hover:bg-brand-primary/5 transition-all group flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-[13px] font-bold italic opacity-60">{{ __('متابعة العملاء والمزودين') }}</p>
            </div>
        </a>
        <a href="{{ route('admin.points-packages.index') }}" class="glass-panel p-6 rounded-3xl border border-[var(--glass-border)] hover:bg-brand-primary/5 transition-all group flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-[13px] font-bold italic opacity-60">{{ __('ضبط الباقات وأسعار الشحن') }}</p>
            </div>
        </a>
        <a href="{{ route('admin.user.index') }}" class="glass-panel p-6 rounded-3xl border border-[var(--glass-border)] hover:bg-brand-primary/5 transition-all group flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-[13px] font-bold italic opacity-60">{{ __('عرض وتعديل بيانات الأعضاء') }}</p>
            </div>
        </a>
        <a href="{{ route('request-complaints.index') }}" class="glass-panel p-6 rounded-3xl border border-[var(--glass-border)] hover:bg-brand-primary/5 transition-all group flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-[13px] font-bold italic opacity-60">{{ __('متابعة وحل نزاعات العمليات') }}</p>
            </div>
        </a>
    </div>
        ...
        <!-- Main Chart Area -->
         <div class="lg:col-span-2 glass-panel p-8 rounded-[2.5rem]">
            <div class="flex justify-between items-center mb-8">
                ...
                <select class="bg-[var(--main-bg)] border-none text-xs font-bold rounded-xl px-4 py-2 outline-none text-[var(--main-text)] shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                    <option>2026</option>
                </select>
            </div>
            <div class="relative h-72 w-full bg-[var(--main-bg)] rounded-2xl flex items-center justify-center border-2 border-dashed border-[var(--glass-border)]">
                <span class="text-sm font-bold text-[var(--text-muted)]">Chart Canvas Integration Area</span>
            </div>
        </div>

        <!-- System Alerts/Tasks -->
         <div class="glass-panel rounded-[2.5rem] overflow-hidden border-none flex flex-col">
            <div class="px-8 py-6 border-b border-[var(--glass-border)]">
                <h3 class="font-black text-lg">{{ __('المهام العاجلة') }}</h3>
            </div>
            <div class="divide-y divide-[var(--glass-border)] flex-1">
                <div class="p-6 hover:bg-[var(--glass-bg)]/50 dark:hover:bg-brand-primary/5 transition group cursor-pointer">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 text-[13px] px-2.5 py-1 rounded-lg font-black whitespace-nowrap inline-flex items-center justify-center">{{ __('3 معلق') }}</span>
                    </div>
                ...
                <div class="p-6 hover:bg-[var(--glass-bg)]/50 dark:hover:bg-brand-primary/5 transition group cursor-pointer">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400 text-[13px] px-2.5 py-1 rounded-lg font-black whitespace-nowrap inline-flex items-center justify-center">{{ __('2 حرج') }}</span>
                    </div>
                    <p class="text-xs font-bold opacity-60">{{ __('نزاع مالي بين عميل ومزود يحتاج تدخل سريع.') }}</p>
                </div>
            </div>
            <button class="m-6 bg-slate-900 text-white py-4 rounded-2xl text-xs font-black transition-all hover:scale-105 shadow-xl">{{ __('مراجعة مركز الشكاوى') }}</button>
        </div>
    </div>
</div>
@endsection
