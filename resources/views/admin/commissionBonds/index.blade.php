...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 font-Cairo text-start">
        <div class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-white dark:border-slate-800/50">
            <div class="flex items-center gap-6 relative z-10 text-start">
                <div class="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-slate-800 dark:text-white shadow-inner font-Cairo text-2xl">📊</div>
                <div class="flex flex-col text-start">
                    <span class="text-3xl font-black font-mono leading-none text-start italic">{{ str_pad($stats['total'], 2, '0', STR_PAD_LEFT) }}</span>
                    <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-60">{{ __('إجمالي السندات') }}</span>
                </div>
            </div>
        </div>
        ...
        <a href="?status=pending" class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-amber-500/10 bg-amber-50/20 dark:bg-amber-950/10">
            <div class="flex items-center gap-6 relative z-10 text-start">
                ...
                <div class="flex flex-col text-start">
                    ...
                    <span class="text-[9px] font-black text-amber-500 uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-80">{{ __('بانتظار الموافقة') }}</span>
                </div>
            </div>
        </a>
        ...
        <div class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-emerald-500/10 bg-emerald-50/20 dark:bg-emerald-950/10">
            <div class="flex items-center gap-6 relative z-10 text-start">
                ...
                <div class="flex flex-col text-start">
                    ...
                    <span class="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-80">{{ __('سندات مقبولة') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-indigo-500/10 bg-indigo-50/20 dark:bg-indigo-950/10">
            <div class="flex items-center gap-6 relative z-10 text-start">
                ...
                <div class="flex flex-col text-start">
                    ...
                    <span class="text-[9px] font-black text-indigo-500 uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-80">{{ __('إجمالي المحصل (ر.س)') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card-premium glass-panel rounded-[4.5rem] overflow-hidden shadow-2xl relative border border-white dark:border-slate-800/50 text-start font-Cairo">
        <div class="px-12 py-10 border-b border-white dark:border-slate-800/50 bg-slate-50/40 dark:bg-slate-950/20 flex justify-between items-center text-start font-Cairo">
            <div class="text-start">
                ...
                <p class="text-[10px] font-black uppercase tracking-[0.3em] mt-2 text-start font-Cairo opacity-60">{{ __('عرض شامل لجميع سندات دفع العمولات المقدمة من المزودين.') }}</p>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-emerald-500/[0.01] transition-all group text-start">
                            ...
                            <td class="px-10 py-7 text-start">
                                <div class="flex flex-col text-start">
                                    ...
                                    <span class="text-[9px] font-black text-slate-400 italic">📅 {{ $bond->created_at->format('Y-m-d H:i') }}</span>
                                </div>
                            </td>
                            <td class="px-10 py-7 text-start">
                                <div class="flex items-center gap-3 text-start">
                                    <div class="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-[11px] font-black group-hover:bg-emerald-600 group-hover:text-white transition-all shadow-inner font-Cairo italic">
                            ...
                            <td class="px-10 py-7 text-center">
                                ...
                                <span class="text-[9px] font-bold opacity-40 ml-1 italic">ر.س</span>
                            </td>
                            <td class="px-10 py-7 text-center">
                            ...
                            <td class="px-10 py-7 text-end">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ asset('storage/' . $bond->image_path) }}" target="_blank" class="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="{{ __('فتح الصورة') }}">
                        ...
                        <tr>
                            <td colspan="6" class="px-10 py-32 text-center">
                                <div class="flex flex-col items-center justify-center gap-6 opacity-30">
                                    <div class="w-24 h-24 bg-slate-50 dark:bg-slate-800 rounded-[2rem] flex items-center justify-center text-6xl shadow-inner animate-pulse italic">🧾</div>
                                    <span class="text-[10px] font-black uppercase tracking-[0.3em] font-Cairo opacity-60 italic">{{ __('لا توجد سندات عمولات حالياً.') }}</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($bonds->hasPages())
        <div class="p-10 border-t border-white dark:border-slate-800/50">
            {{ $bonds->links() }}
        </div>
        @endif
    </div>
