...
<div class="h-full flex flex-col gap-8 font-Cairo py-6 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        <div class="space-y-2">
            <h1 class="text-4xl font-black tracking-tighter italic text-slate-800 dark:text-white uppercase leading-tight">
                {{ __('مركز الترويج') }} <span class="text-brand-primary italic">{{ __('الذكي') }}</span>
            </h1>
            <p class="text-slate-500 dark:text-slate-400 font-bold text-sm flex items-center gap-2 italic">
        ...
        <div class="flex items-center gap-3">
            <button onclick="exportTableToCSV('ads-table', 'advertisements-report.csv')" 
                    class="group px-6 py-4 bg-white dark:bg-white/5 text-slate-700 dark:text-white rounded-2xl font-black text-sm transition-all duration-500 hover:bg-slate-50 border border-slate-100 dark:border-white/5 flex items-center gap-2">
            ...
            <a href="{{ route('advertisements.create') }}" 
               class="group relative px-8 py-4 bg-brand-primary text-white rounded-2xl font-black text-sm transition-all duration-500 hover:scale-105 active:scale-95 shadow-xl shadow-brand-primary/20 overflow-hidden">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-2">
        ...
        <div class="card-premium p-6 flex items-center gap-6 group hover:border-indigo-500/50 transition-all duration-500">
            ...
            <div>
                <span class="block text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">{{ __('إجمالي الحملات') }}</span>
                <span class="text-3xl font-black text-slate-800 dark:text-white tabular-nums">{{ $stats['total'] }}</span>
            </div>
        </div>
        ...
        <div class="card-premium p-6 flex items-center gap-6 group hover:border-emerald-500/50 transition-all duration-500">
            ...
            <div>
                <span class="block text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">{{ __('نشط حالياً') }}</span>
        ...
        <div class="card-premium p-6 flex items-center gap-6 group hover:border-amber-500/50 transition-all duration-500">
            ...
            <div>
                <span class="block text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">{{ __('إجمالي الظهور') }}</span>
                <span class="text-3xl font-black text-slate-800 dark:text-white tabular-nums">{{ number_format($stats['views']) }}</span>
            </div>
        </div>
        ...
        <div class="card-premium p-6 flex items-center gap-6 group hover:border-brand-primary/50 transition-all duration-500">
            ...
            <div>
                <span class="block text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">{{ __('إجمالي التفاعل') }}</span>
    ...
    @if($advertisements->isEmpty())
        ...
        <div class="flex-1 flex flex-col items-center justify-center py-20 px-4">
            ...
            <h3 class="text-2xl font-black text-slate-800 dark:text-white mb-2 italic">{{ __('لا توجد إعلانات حالياً') }}</h3>
            <p class="text-slate-500 dark:text-slate-400 font-bold max-w-md text-center italic">{{ __('ابدأ بإضافة أول بنر إعلاني ليظهر لمستخدمي تطبيق الجوال فوراً') }}</p>
        </div>
    @else
        <!-- Ads Grid -->
                ...
                <div class="group relative card-premium overflow-hidden">
                    ...
                    <div class="aspect-[16/9] relative overflow-hidden">
                        ...
                        <div class="absolute top-6 left-6 flex flex-col gap-2">
                            <span class="badge-soft shadow-xl !bg-white/90 dark:!bg-slate-900/90 backdrop-blur-md">
                                {{ __($ad->type) }}
                            </span>
                        ...
                        <div class="absolute inset-0 bg-brand-primary/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center backdrop-blur-sm">
                            <div class="flex items-center gap-4">
                                <a href="{{ route('advertisements.edit', $ad->id) }}" 
                                   class="w-12 h-12 bg-white text-brand-primary rounded-2xl flex items-center justify-center hover:scale-110 transition-transform shadow-xl">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                    </svg>
                                </a>
                                <button onclick="confirmAction('delete-form-{{ $ad->id }}', { isDanger: true })"
                                        class="w-12 h-12 bg-rose-500 text-white rounded-2xl flex items-center justify-center hover:scale-110 transition-transform shadow-xl">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-xl font-black text-slate-800 dark:text-white line-clamp-1 italic tracking-tight uppercase">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <!-- Targeting Info -->
                            <div class="flex flex-col gap-1.5 p-4 rounded-3xl bg-slate-100/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 shadow-sm transition-colors duration-500">
                                <div class="flex items-center gap-2 text-[8px] font-black uppercase tracking-widest opacity-40 italic text-slate-500 dark:text-slate-400">
                                ...
                                <div class="flex flex-col min-h-[30px] justify-center">
                                    <span class="text-[11px] font-black text-brand-primary italic uppercase leading-none">{{ __($ad->target_type) }}</span>
                                    @if($ad->target_type == 'service' || $ad->target_type == 'category')
                                        <span class="text-[9px] font-bold text-slate-500 dark:text-slate-400 line-clamp-1 mt-1">{{ $ad->target?->name ?? 'N/A' }}</span>
                                    @elseif($ad->target_type == 'external')
                                        <span class="text-[9px] font-bold text-slate-500 dark:text-slate-400 line-clamp-1 mt-1">{{ $ad->external_link }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col gap-1.5 p-4 rounded-3xl bg-slate-100/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 shadow-sm transition-colors duration-500">
                                ...
                                <div class="flex items-center min-h-[30px]">
                                    <span class="text-[16px] font-black text-slate-800 dark:text-white italic">#{{ $ad->sort_order }}</span>
                                </div>
                            </div>
                        </div>

                        <!-- Timeline & Individual Metrics -->
                        <div class="space-y-4 bg-slate-100/30 dark:bg-white/5 p-4 rounded-3xl border border-transparent dark:border-white/5">
                            <!-- Campaign Schedule -->
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-[8px] font-black uppercase tracking-wider opacity-40 italic text-slate-500 dark:text-slate-400 mb-0.5">{{ __('الفترة الزمنية') }}</span>
                                    <span class="text-[10px] font-black text-slate-600 dark:text-slate-200 tracking-tighter">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-slate-200/50 dark:border-white/5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[8px] font-bold opacity-40 uppercase">{{ __('مشاهدة') }}</span>
                                </div>
                                <div class="flex items-center gap-2 text-brand-primary">
                                    ...
                                    <span class="text-[8px] font-bold opacity-60 uppercase">{{ __('نقرة') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
