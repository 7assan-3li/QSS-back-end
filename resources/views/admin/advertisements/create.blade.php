...
<div class="max-w-5xl mx-auto py-10 px-4 font-Cairo animate-in fade-in slide-in-from-bottom-8 duration-700" x-data="{ targetType: 'none', imagePreview: null }">
    ...
    <div class="mb-10 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <span class="text-[13px] font-black uppercase tracking-[0.3em] opacity-40">{{ __('بناء حملة جديدة') }}</span>
        </div>
    </div>
    ...
    <form action="{{ route('advertisements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-8">
                        ...
                        <div class="aspect-[16/9] rounded-3xl bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 border-2 border-dashed border-[var(--glass-border)] flex flex-col items-center justify-center overflow-hidden transition-all duration-500 group-hover:border-brand-primary/50 relative">
                            <template x-if="!imagePreview">
                                <div class="flex flex-col items-center text-center p-6">
                                    ...
                                    <span class="text-xs font-black text-[var(--text-muted)] italic block mb-1">{{ __('اختر صورة الإعلان') }}</span>
                                    <span class="text-[12px] font-bold text-[var(--text-muted)] opacity-60 italic">{{ __('المقياس المفضل 16:9') }}</span>
                                </div>
                            </template>
                ...
                <!-- Display Settings -->
                <div class="bg-[var(--glass-bg)] rounded-[2.5rem] border border-[var(--glass-border)] p-8 transition-colors duration-500">
                    <h3 class="text-lg font-black text-[var(--main-text)] mb-6 italic">{{ __('إرشادات النشر') }}</h3>
                    <div class="space-y-4">
                        <div class="flex items-start gap-4">
                            ...
                            <p class="text-[14px] font-bold text-[var(--text-muted)] italic">{{ __('سيظهر الإعلان فوراً بمجرد تفعيل حالة النشاط وبلوغ تاريخ البداية.') }}</p>
                        </div>
                        <div class="flex items-start gap-4">
                            ...
                            <p class="text-[14px] font-bold text-[var(--text-muted)] italic">{{ __('تأكد من دقة الرابط الخارجي أو اختيار الخدمة الصحيحة للتوجيه.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="card-premium p-10 transition-colors duration-500">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" placeholder="{{ __('مثال: عرض اليوم الوطني') }}" class="input-premium placeholder:opacity-30">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" placeholder="{{ __('اكتب تفاصيل العرض هنا ليراها المستخدم...') }}" class="input-premium placeholder:opacity-30 min-h-[100px] py-4">{{ old('description') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="input-premium italic">
                        </div>
                ...
                <div class="card-premium p-10 transition-colors duration-500">
                    <h3 class="text-xl font-black text-[var(--main-text)] mb-8 flex items-center gap-4 italic uppercase">
                        ...
                        {{ __('توجيه الروابط المباشر') }} (Deep Linking)
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="md:col-span-2 animate-in fade-in duration-500" x-show="targetType === 'external'">
                            ...
                            <input type="url" name="external_link" placeholder="https://example.com/promotion" class="input-premium placeholder:opacity-30">
                        </div>
                    </div>
                </div>
                ...
                <div class="card-premium p-10 transition-colors duration-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <input type="datetime-local" name="starts_at" value="{{ old('starts_at', now()->format('Y-m-d\TH:i')) }}" class="input-premium italic tracking-tight">
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="ends_at" value="{{ old('ends_at') }}" class="input-premium italic tracking-tight">
                        </div>
                    </div>
                </div>
