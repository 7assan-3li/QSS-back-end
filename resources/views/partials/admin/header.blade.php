<header class="h-24 flex items-center justify-between px-12 z-40 transition-all duration-700 bg-[var(--glass-bg)] border-b border-slate-200 dark:border-white/5 sticky top-0 font-Cairo">
    ...
    <div class="flex flex-col text-start font-Cairo">
        ...
        <div class="flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.3em] mt-2 text-start font-Cairo italic opacity-60">
    ...
    <div class="flex items-center gap-8 text-start font-Cairo">
        ...
        <div class="hidden lg:flex items-center gap-5 px-6 py-2.5 bg-brand-primary/5 rounded-2xl border border-brand-primary/10">
            <div class="flex flex-col items-end">
                <span class="text-[9px] font-extrabold uppercase tracking-[0.2em] italic opacity-60">{{ __('حالة الشبكة') }}</span>
                <span class="text-[10px] font-extrabold text-emerald-600 italic flex items-center gap-1">{{ __('متصل بالخادم') }} <span class="text-[8px] mt-0.5">🟢</span></span>
            </div>
        </div>
        ...
        <div class="flex items-center bg-brand-primary/5 p-1.5 rounded-[1.25rem] border border-brand-primary/10">
            <button onclick="toggleTheme()" class="w-12 h-12 flex items-center justify-center rounded-xl hover:bg-white dark:hover:bg-white/10 hover:text-brand-primary transition-all duration-500 group">
            ...
            <a href="{{ route('lang.switch', app()->getLocale() == 'ar' ? 'en' : 'ar') }}" 
               class="w-12 h-12 flex items-center justify-center rounded-xl font-black text-[10px] hover:bg-white dark:hover:bg-white/10 hover:text-brand-primary transition-all duration-500 italic uppercase">
        ...
        <div class="flex items-center gap-5 pl-4 group cursor-pointer">
            <div class="text-end hidden sm:block">
                ...
                <p class="text-[10px] font-black tracking-[0.2em] mt-1 italic uppercase subpixel-antialiased opacity-60">Root Admin</p>
            </div>
            <div class="relative">
