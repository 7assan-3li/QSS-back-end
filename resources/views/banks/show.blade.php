...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start font-Cairo">
        ...
        <div class="flex items-center gap-4 text-start font-Cairo">
            <a href="{{ route('banks.edit', $bank->id) }}" class="px-10 py-5 bg-gradient-to-r from-brand-primary to-indigo-600 text-white rounded-[2rem] text-[11px] font-black uppercase tracking-[0.2em] shadow-xl shadow-brand-primary/20 hover:scale-[1.05] active:scale-95 transition-all font-Cairo flex items-center gap-3 text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 font-Cairo text-start">
        ...
        <div class="lg:col-span-8 space-y-12 text-start font-Cairo">
            ...
            <div class="card-premium glass-panel p-14 rounded-[2rem] shadow-2xl relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
                ...
                <div class="flex items-center gap-5 mb-14 text-start font-Cairo">
                    ...
                    <h4 class="text-2xl font-black text-slate-800 dark:text-white font-Cairo text-start italic">{{ __('البيانات الأساسية') }}</h4>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10 text-start font-Cairo">
                    <!-- Bank Identity Node -->
                    <div class="flex items-center gap-6 group p-8 bg-slate-50/50 dark:bg-slate-950/40 rounded-[1.5rem] border border-slate-100 dark:border-slate-800/60 shadow-inner group transition-all font-Cairo">
                        <div class="w-16 h-16 bg-white dark:bg-slate-900 text-emerald-500 rounded-[1.2rem] flex items-center justify-center text-2xl shadow-xl group-hover:scale-110 group-hover:rotate-6 transition-all font-Cairo italic">🏦</div>
                        <div class="flex flex-col text-start font-Cairo">
                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2 font-Cairo text-start">{{ __('المسمى المصرفي') }}</span>
                            <span class="text-sm font-black text-slate-800 dark:text-white font-Cairo italic text-start">{{ $bank->bank_name }}</span>
                        </div>
                    </div>

                    <!-- Certification Node -->
                    <div class="flex items-center gap-6 group p-8 bg-slate-50/50 dark:bg-slate-950/40 rounded-[1.5rem] border border-slate-100 dark:border-slate-800/60 shadow-inner group transition-all font-Cairo">
                        <div class="w-16 h-16 bg-white dark:bg-slate-900 text-indigo-500 rounded-[1.2rem] flex items-center justify-center text-2xl shadow-xl group-hover:scale-110 group-hover:rotate-6 transition-all font-Cairo italic">🗓️</div>
                        <div class="flex flex-col text-start font-Cairo">
                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2 font-Cairo text-start">{{ __('تاريخ الإضافة') }}</span>
                            <span class="text-sm font-black text-slate-800 dark:text-white font-mono tracking-widest text-start italic font-Cairo">{{ $bank->created_at->format('Y-m-d') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-premium glass-panel p-14 rounded-[2rem] shadow-2xl relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
                <div class="flex items-center gap-5 mb-10 text-start font-Cairo">
                    ...
                    <h4 class="text-2xl font-black text-slate-800 dark:text-white font-Cairo text-start italic">{{ __('وصف البنك') }}</h4>
                </div>
                <div class="p-10 bg-slate-50/50 dark:bg-slate-950/40 rounded-[1.5rem] border border-slate-100 dark:border-slate-800/80 shadow-inner text-start font-Cairo">
                    <p class="text-lg font-bold text-slate-600 dark:text-slate-300 leading-[2] font-Cairo text-start italic">
                         " {{ $bank->description ?? __('لا توجد بيانات وصفية لهذا البنك حالياً.') }} "
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
            ...
            <div class="card-premium glass-panel p-8 rounded-[2rem] shadow-2xl relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
                <div class="absolute top-0 left-0 w-32 h-32 bg-emerald-500/[0.04] rounded-br-[4rem] -ml-10 -mt-10 font-Cairo"></div>
                
                <h4 class="font-black text-slate-500 text-[10px] uppercase tracking-[0.3em] mb-10 px-6 font-Cairo text-start italic font-Cairo">💎 {{ __('شعار البنك المعتمد') }}</h4>

                <div class="relative w-full aspect-square rounded-[2rem] border-8 border-white dark:border-slate-900 shadow-[0_40px_80px_-15px_rgba(0,0,0,0.3)] bg-slate-50 dark:bg-slate-950 overflow-hidden group font-Cairo">
                    @if($bank->image_path)
                    ...
                    @else
                        <div class="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-900 dark:to-slate-900 text-slate-300 gap-8 font-Cairo">
                            <div class="w-24 h-24 bg-white/50 dark:bg-white/5 rounded-[1.2rem] flex items-center justify-center shadow-inner font-Cairo">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-[0.4em] font-Cairo opacity-40 font-Cairo">NO_LOGO</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="p-10 bg-slate-950 rounded-[2rem] border border-slate-900 shadow-3xl text-start font-Cairo">
                <h4 class="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] text-center mb-10 font-Cairo italic">{{ __('إجراءات إدارية') }}</h4>
                <div class="space-y-6 text-start font-Cairo">
                    <a href="{{ route('banks.edit', $bank->id) }}" class="w-full py-6 bg-white/5 text-brand-primary border border-brand-primary/20 rounded-[1.5rem] text-[11px] font-black flex items-center justify-center gap-4 hover:bg-brand-primary hover:text-white transition-all shadow-xl shadow-brand-primary/5 font-Cairo italic font-Cairo">
                        <svg class="w-5 h-5 font-black uppercase" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        {{ __('تعديل بيانات البنك') }}
                    </a>
                    ...
                    <form id="delete-bank-{{ $bank->id }}" action="{{ route('banks.destroy', $bank->id) }}" method="POST" class="w-full font-Cairo text-start">
                        ...
                        <button type="button" 
                            ...
                            })" class="w-full py-6 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-[1.5rem] text-[11px] font-black flex items-center justify-center gap-4 hover:bg-rose-500 hover:text-white transition-all shadow-xl shadow-rose-500/5 font-Cairo uppercase tracking-[0.2em] italic font-Cairo">
