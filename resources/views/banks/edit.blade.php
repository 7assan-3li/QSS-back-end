        ...
        <div class="text-start font-Cairo">
            <div class="flex items-center gap-5 mb-5 text-start font-Cairo">
                ...
                <h3 class="font-black text-3xl text-slate-800 dark:text-white flex items-center gap-4 text-start font-Cairo">
                    <span class="w-12 h-12 bg-amber-500/10 rounded-2xl flex items-center justify-center text-amber-600 text-2xl font-Cairo shadow-lg shadow-amber-500/5 font-Cairo underline-offset-8 italic">✍️</span>
            ...
            <div class="lg:col-span-8 space-y-12">
                <div class="card-premium glass-panel p-14 rounded-[4.5rem] shadow-2xl relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
                    ...
                    <div class="flex items-center gap-5 mb-14 text-start font-Cairo">
                        ...
                        <h4 class="text-2xl font-black text-slate-800 dark:text-white font-Cairo text-start italic">{{ __('تعديل بيانات البنك') }}</h4>
                    </div>
                    ...
                    <div class="space-y-12 relative z-10 text-start font-Cairo">
                        ...
                        <div class="flex flex-col gap-5 text-start font-Cairo">
                            ...
                            <div class="relative group font-Cairo">
                                <input type="text" name="bank_name" value="{{ old('bank_name', $bank->bank_name) }}" placeholder="{{ __('أدخل المسمى الجديد...') }}"
                        ...
                        <div class="flex flex-col gap-5 text-start font-Cairo">
                            ...
                            <textarea name="description" rows="6" placeholder="{{ __('اكتب التعديلات المطلوبة للوصف...') }}" 
                                class="w-full bg-slate-50/50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-[3.5rem] px-10 py-8 text-sm font-black text-slate-700 dark:text-white focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all outline-none resize-none leading-relaxed italic text-start font-Cairo shadow-inner">{{ old('description', $bank->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-12">
                ...
                <div class="card-premium glass-panel p-10 rounded-[4rem] shadow-2xl border border-white dark:border-white/5 text-start font-Cairo">
                    <div class="flex items-center gap-5 mb-10 text-start font-Cairo">
                        ...
                        <h4 class="text-[10px] font-black text-slate-800 dark:text-white font-Cairo uppercase tracking-[0.3em] text-start italic font-Cairo">{{ __('تحديث شعار البنك') }}</h4>
                    </div>
                    
                    <div class="relative group font-Cairo">
                        <div id="preview" class="w-full aspect-square rounded-[3.5rem] border-8 border-white dark:border-slate-900 shadow-[0_45px_90px_-20px_rgba(0,0,0,0.3)] bg-slate-50 dark:bg-slate-950 overflow-hidden transition-all group-hover:rotate-3 p-1 font-Cairo">
                            @if($bank->image_path)
                            ...
                            @else
                                <div class="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-6 font-Cairo">
                                    ...
                                    <span class="text-[9px] font-black uppercase tracking-[0.3em] font-Cairo opacity-50 font-Cairo italic">NO_LOGO</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <label class="absolute inset-x-0 bottom-6 flex justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 font-Cairo">
                            ...
                            <div class="px-10 py-5 bg-brand-primary text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl backdrop-blur-md bg-brand-primary/95 font-Cairo italic">{{ __('تغيير الصورة') }}</div>
                        </label>
                    </div>
                </div>
                ...
                <div class="p-10 bg-slate-950 rounded-[4rem] border border-slate-900 shadow-3xl text-start font-Cairo">
                    <button type="submit" class="w-full py-7 bg-gradient-to-r from-brand-primary to-indigo-600 text-white rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-[0_25px_50px_-15px_rgba(79,70,229,0.4)] hover:scale-[1.05] active:scale-95 transition-all duration-500 font-Cairo flex items-center justify-center gap-5 italic text-start font-Cairo">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                        {{ __('حفظ التعديلات') }}
                    </button>
                    <a href="{{ route('banks.show', $bank->id) }}" class="w-full mt-6 py-6 border border-white/5 text-slate-500 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.4em] flex items-center justify-center hover:text-white hover:bg-white/5 transition-all font-Cairo italic font-Cairo">{{ __('إلغاء') }}</a>
                </div>
            </div>
        </div>
    </form>
