...
<div class="space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        ...
        <a href="{{ route('categories.create') }}" class="group inline-flex items-center justify-center gap-3 px-8 py-4 bg-brand-primary text-white rounded-[1.75rem] text-xs font-black shadow-[0_20px_40px_-5px_rgba(var(--brand-primary-rgb),0.3)] hover:scale-[1.05] active:scale-95 transition-all font-Cairo">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-start">
        <!-- Total Terminal -->
        <div class="card-premium glass-panel p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group border border-white dark:border-slate-800/50 text-start">
    ...
    @if ($categories->count())
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 text-start font-Cairo">
            @foreach ($categories->whereNull('category_id') as $category)
                <div class="card-premium glass-panel rounded-[3rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] relative group overflow-hidden border border-white dark:border-slate-800/50 transform transition-all duration-700 hover:-translate-y-4 hover:shadow-2xl text-start">
                    ...
                    <div class="h-60 relative overflow-hidden text-start">
                        ...
                        <div class="absolute bottom-8 right-8 left-8 text-start">
                            <div class="flex flex-col gap-1 text-start">
                                <span class="text-[9px] font-black text-brand-primary uppercase tracking-[0.3em] font-Cairo mb-1 drop-shadow-lg">{{ __('تصنيف رئيسي') }}</span>
                    ...
                    <div class="p-10 space-y-8 text-start">
                        ...
                        <div class="flex items-center gap-3 pt-6 border-t border-slate-100 dark:border-slate-800/50 text-start">
    ...
    @else
        <div class="card-premium glass-panel p-24 rounded-[4rem] text-center font-Cairo shadow-2xl relative overflow-hidden border border-dashed border-slate-200 dark:border-slate-800">
            ...
            <div class="relative z-10 text-center">
                ...
                <a href="{{ route('categories.create') }}" class="mt-10 inline-flex items-center gap-3 px-10 py-5 bg-brand-primary text-white rounded-[1.75rem] text-xs font-black shadow-2xl shadow-brand-primary/30 hover:scale-[1.05] active:scale-95 transition-all">
