...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
        ...
        <div class="flex flex-wrap items-center gap-4 text-start">
            <form id="delete-category-{{ $category->id }}" action="{{ route('categories.destroy', $category) }}" method="POST" class="text-start">
                ...
                <button type="button" 
                    ...
                    class="group px-8 py-4 bg-rose-500/10 text-rose-600 border border-rose-500/20 rounded-2xl text-[10px] font-black hover:bg-rose-600 hover:text-white transition-all font-Cairo shadow-sm flex items-center gap-3 text-start">
                    <span class="opacity-0 group-hover:opacity-100 transition-opacity italic">🗑️</span>
                    {{ __('حذف القسم نهائياً') }}
                </button>
            </form>
            <a href="{{ route('categories.index') }}" class="px-6 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl text-[10px] font-black hover:bg-brand-primary hover:text-white transition-all font-Cairo">
                {{ __('العودة للقائمة') }}
            </a>
        </div>
    </div>
    ...
    <!-- Edit Category Form -->
    <div class="card-premium glass-panel p-10 md:p-16 rounded-[4.5rem] shadow-[0_60px_120px_-30px_rgba(0,0,0,0.15)] relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
        <form action="{{ route('categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-14 text-start">
            ...
            <div class="space-y-10 text-start">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 text-start">
                    ...
                    <div class="space-y-4 text-start">
                        ...
                        @error('name')
                            <span class="text-[9px] font-black text-rose-500 px-4 mt-2 block font-Cairo text-start">⚠️ {{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="space-y-10 pt-12 border-t border-slate-100 dark:border-slate-800 text-start">
                ...
                <div class="flex flex-col xl:flex-row gap-12 items-center text-start">
                    ...
                    <div class="w-full xl:w-80 h-72 rounded-[4rem] border-2 border-slate-100 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 flex items-center justify-center overflow-hidden relative group shadow-2xl text-center">
            ...
            <div class="pt-12 flex flex-col sm:flex-row items-center gap-6 text-start">
                <button type="submit" class="w-full sm:w-auto px-16 py-6 bg-brand-primary text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-brand-primary/30 hover:scale-[1.05] active:scale-95 transition-all font-Cairo flex items-center justify-center gap-4">
                    {{ __('حفظ التغييرات 💾') }}
                </button>
                <a href="{{ route('categories.index') }}" class="w-full sm:w-auto px-16 py-6 border-2 border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] hover:bg-slate-100 dark:hover:bg-slate-900 transition-all text-center font-Cairo">
                    {{ __('إلغاء') }}
                </a>
            </div>
        </form>
