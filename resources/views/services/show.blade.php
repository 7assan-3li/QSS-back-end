...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo pb-24 print:p-0 print:m-0">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 px-4 md:px-0 text-start group print:flex-row print:justify-between font-Cairo">
            ...
            <h3 class="font-black text-4xl flex items-center gap-6 text-start font-Cairo leading-tight">
                <div class="relative group/icon print:hidden">
                    @if($service->image_path)
                        <img src="{{ asset('storage/' . $service->image_path) }}" class="w-20 h-20 rounded-[2.2rem] object-cover shadow-2xl border-4 border-[var(--glass-border)] transition-all group-hover/icon:scale-110 group-hover/icon:-rotate-6" alt="{{ $service->name }}">
                    @else
                    ...
                    @endif
                    <div class="absolute -bottom-1 -right-1 w-8 h-8 bg-[var(--glass-bg)] rounded-full flex items-center justify-center shadow-lg border-2 border-[var(--glass-border)]">
                        <div class="w-3.5 h-3.5 {{ $service->is_active ? 'bg-emerald-500' : 'bg-rose-500' }} rounded-full {{ $service->is_active ? 'animate-ping' : '' }}"></div>
                    </div>
                </div>
                <div class="flex flex-col text-start">
                    <span class="italic text-[var(--main-text)]">{{ $service->name }}</span>
                    <div class="flex items-center gap-3 text-[13px] font-black text-[var(--text-muted)] mt-2 tracking-[0.3em] uppercase opacity-60 font-Cairo">
        ...
        <div class="flex items-center gap-5 print:hidden">
            <a href="{{ route('services.index') }}" class="h-14 px-8 bg-[var(--glass-border)] text-[var(--text-secondary)] rounded-2xl font-black text-[14px] uppercase tracking-widest flex items-center gap-3 transition-all hover:bg-slate-200">
            ...
            <a href="{{ route('services.edit', $service->id) }}" class="h-14 px-10 bg-brand-primary text-white rounded-2xl font-black text-[14px] uppercase tracking-[0.2em] flex items-center gap-4 transition-all shadow-2xl shadow-brand-primary/30 hover:scale-[1.05] italic italic group">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-4 md:px-0">
        <!-- Requests KPI -->
        <div class="card-premium glass-panel p-10 border-l-8 border-indigo-500 relative overflow-hidden group shadow-xl bg-[var(--glass-bg)]/40">
            <div class="relative z-10 text-start">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-3 block opacity-70 group-hover:text-indigo-500 transition-colors">{{ __('إجمالي الطلبات') }}</span>
                <div class="flex items-baseline gap-4 text-start font-mono text-start">
                    ...
                    <span class="text-[13px] font-black opacity-30 uppercase">{{ __('طلب') }}</span>
                </div>
            </div>
        ...
        <div class="card-premium glass-panel p-10 border-l-8 border-emerald-500 relative overflow-hidden group shadow-xl bg-[var(--glass-bg)]/40 font-Cairo text-start">
            <div class="relative z-10 text-start">
                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-3 block opacity-70 group-hover:text-emerald-500 transition-colors uppercase italic">{{ __('طلبات ناجزة') }}</span>
                <div class="flex items-baseline gap-4 text-start font-mono text-start">
                    <span class="text-5xl font-black italic text-emerald-600">{{ number_format($stats['completed_requests'], 0) }}</span>
                    <span class="text-[13px] font-black opacity-30 uppercase underline italic">{{ __('تم التسليم') }}</span>
                </div>
            </div>
        ...
        <!-- Revenue KPI -->
        <div class="card-premium glass-panel p-10 border-l-8 border-amber-500 relative overflow-hidden group shadow-xl bg-[var(--glass-bg)]/40 text-start font-Cairo">
        ...
        <div class="card-premium glass-panel p-10 border-l-8 border-brand-primary relative overflow-hidden group shadow-xl bg-[var(--glass-bg)] text-[var(--main-text)] border border-[var(--glass-border)] font-Cairo text-start">
            <div class="relative z-10 text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 px-4 md:px-0">
        ...
        <div class="lg:col-span-4 space-y-10">
            ...
            <div class="card-premium glass-panel p-10 rounded-[3rem] shadow-2xl relative overflow-hidden text-start font-Cairo bg-[var(--glass-bg)] border border-white/10">
                <h5 class="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.4em] mb-10 opacity-70 italic">{{ __('هوية ومسؤول الخدمة') }}</h5>
                
                <div class="space-y-10 text-start font-Cairo">
                    <div class="flex items-center gap-6 text-start">
                        <div class="w-16 h-16 bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 rounded-[1.5rem] flex items-center justify-center text-2xl shadow-inner font-Cairo opacity-70">
                             {{ mb_substr($service->provider->name ?? '?', 0, 1) }}
                        </div>
                        <div class="flex flex-col text-start">
                            <span class="text-[12px] font-black text-brand-primary uppercase tracking-widest italic leading-tight">{{ __('مزود الخدمة المعتمد') }}</span>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-start font-Cairo">
                        <div class="p-6 bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 rounded-3xl border border-[var(--glass-border)] text-start font-Cairo">
                            <span class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-widest italic block mb-2">{{ __('الحالة') }}</span>
                            <span class="inline-flex items-center gap-2 text-[13px] font-black {{ $service->is_available ? 'text-emerald-500' : 'text-rose-500' }} font-Cairo italic leading-tight uppercase">
                        ...
                        <div class="p-6 bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 rounded-3xl border border-[var(--glass-border)] text-start font-Cairo">
                            <span class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-widest italic block mb-2">{{ __('التسعير') }}</span>
                            <span class="text-[13px] font-black text-[var(--main-text)] font-Cairo italic leading-tight uppercase">{{ $service->distance_based_price ? __('حسب المسافة') : __('سعر ثابت') }}</span>
                        </div>
                    </div>

                    <div class="pt-8 border-t border-[var(--glass-border)] flex items-center justify-between text-start font-Cairo">
                        <div class="text-start">
                            <span class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-widest italic block mb-2">{{ __('تقييم الجودة العام') }}</span>
                            <div class="flex items-center gap-1.5 text-start">
                                @for($i=1; $i<=5; $i++)
                                    <svg class="w-3.5 h-3.5 {{ $i <= $stats['average_rating'] ? 'text-amber-400' : 'text-slate-200 ' }} fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                @endfor
                            </div>
                        </div>
            ...
            <div class="card-premium glass-panel p-10 rounded-[3rem] shadow-xl border border-white/10 text-start font-Cairo">
                 <h5 class="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.4em] mb-10 opacity-70 italic">{{ __('تفاصيل التسعير والعمولات') }}</h5>
                 <div class="space-y-6 text-start font-Cairo">
                    ...
                    <div class="pt-6 border-t border-[var(--glass-border)] flex justify-between items-center text-start font-Cairo">
                        <span class="text-[14px] font-bold text-[var(--text-muted)] opacity-60 font-Cairo italic">{{ __('نسبة الدفعة المقدمة') }}</span>
                        <span class="px-4 py-1.5 bg-brand-primary/10 text-brand-primary rounded-xl text-xs font-black font-mono italic whitespace-nowrap inline-flex items-center justify-center">{{ $service->required_partial_percentage }}%</span>
                    </div>
                 </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-8 space-y-10">
            ...
            <div class="card-premium glass-panel p-12 md:p-16 rounded-[4.5rem] shadow-2xl relative border border-white/10 text-start font-Cairo">
                ...
                <div class="relative text-start font-Cairo">
                    ...
                    <p class="text-xl font-bold leading-[2.2] pr-12 text-[var(--text-secondary)] italic text-start font-Cairo opacity-80">
            ...
            <div class="card-premium glass-panel rounded-[4.5rem] overflow-hidden shadow-2xl border border-white/10 flex flex-col font-Cairo">
                <div class="p-12 border-b border-[var(--glass-border)] flex justify-between items-center text-start group font-Cairo">
                    <div class="text-start font-Cairo">
                        <h4 class="text-2xl font-black italic text-start font-Cairo">{{ __('سجل مراجعات الجودة') }}</h4>
                        <span class="text-[13px] font-black opacity-30 uppercase tracking-[0.3em] font-Cairo mt-2 italic block">Live Customer Feedback Loop</span>
                    </div>
                    ...
                    @else
                        <div class="space-y-10 text-start font-Cairo italic">
                            ...
                            <div class="flex gap-8 group/item font-Cairo text-start italic">
                                <div class="w-14 h-14 rounded-2xl bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 flex items-center justify-center text-xl font-black shrink-0 shadow-lg font-Cairo italic text-start">
                                ...
                                <div class="flex-1 space-y-4 text-start font-Cairo italic">
                                    <div class="flex justify-between items-center text-start font-Cairo">
                                        <div class="text-start font-Cairo">
                                            ...
                                            <span class="text-[12px] font-black text-[var(--text-muted)] mt-1 block uppercase tracking-tighter opacity-70">{{ $review->created_at->diffForHumans() }}</span>
                                        </div>
                                    ...
                                    <p class="text-[13px] font-bold text-[var(--text-muted)] leading-relaxed italic bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/[0.02] p-6 rounded-[2rem] border border-[var(--glass-border)]">
                                        {{ $review->comment ?? '---' }}
                                    </p>
                                </div>
                            </div>
            ...
            <div class="card-premium glass-panel rounded-[4.5rem] overflow-hidden shadow-2xl border border-white/10 font-Cairo">
                 <div class="p-12 border-b border-[var(--glass-border)] flex justify-between items-center text-start font-Cairo">
                    <div class="text-start font-Cairo">
                        <h4 class="text-2xl font-black italic text-start font-Cairo">{{ __('الخدمات الفرعية التابعة') }}</h4>
                        <span class="text-[13px] font-black opacity-30 uppercase tracking-[0.3em] font-Cairo mt-2 italic block">{{ $service->children->count() }} Sub-services active</span>
                    </div>
                </div>
                <div class="overflow-x-auto text-start font-Cairo">
                    <table class="w-full text-start font-Cairo">
                        <tbody class="divide-y divide-[var(--glass-border)] dark:divide-white/3 font-Cairo">
                            @foreach($service->children as $child)
                            <tr class="hover:bg-[var(--glass-bg)]/50 dark:hover:bg-[var(--glass-bg)]/[0.02] transition-all group/item font-Cairo">
                                <td class="px-12 py-10 text-start font-Cairo">
                                    <div class="flex flex-col text-start">
                                        ...
                                        <p class="text-[13px] font-black text-[var(--text-muted)] mt-2 font-Cairo opacity-50">{{ Str::limit($child->description, 60) }}</p>
                                    </div>
                                </td>
                                <td class="px-12 py-10 text-start font-Cairo">
                                ...
                                <td class="px-12 py-10 text-end font-Cairo">
                                    <div class="flex justify-end gap-3 font-Cairo italic">
                                        <a href="{{ route('services.show', $child->id) }}" class="p-4 bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/5 rounded-2xl text-[var(--text-muted)] hover:text-brand-primary hover:bg-brand-primary/10 transition-all shadow-sm">
