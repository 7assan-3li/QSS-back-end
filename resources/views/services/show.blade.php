...
<div class="max-w-6xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 text-start">
        <div class="text-start">
            ...
            <div class="flex items-center gap-3 text-[10px] font-black text-slate-400 mt-3 mr-20 uppercase tracking-[0.2em] font-Cairo text-start">
                ...
                <svg class="w-2 h-2 rtl:rotate-0 ltr:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
                <span class="text-brand-primary">{{ __('هوية الخدمة') }}</span>
            </div>
        </div>
        ...
        <div class="p-12 md:p-16 space-y-14 text-start">
            ...
            <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-10 pb-12 border-b border-slate-100 dark:border-slate-800/50 text-start">
                <div class="space-y-4 text-start">
                    <span class="px-4 py-2 bg-indigo-500/10 text-indigo-500 rounded-xl text-[9px] font-black uppercase tracking-[0.3em] font-Cairo inline-block text-start">
                        {{ $service->category->name ?? __('خدمة غير مصنفة') }}
                    </span>
                    <h2 class="text-4xl font-black text-slate-800 dark:text-white leading-[1.2] font-Cairo text-start">{{ $service->name }}</h2>
                </div>
                ...
                <div class="flex flex-wrap gap-6 text-start">
                    <div class="px-8 py-5 bg-brand-primary/[0.03] rounded-[2.5rem] border border-brand-primary/10 text-center relative group overflow-hidden">
                        ...
                        <span class="block text-[8px] font-black text-brand-primary uppercase tracking-[0.3em] mb-2 font-Cairo relative z-10">{{ __('السعر الحالي') }}</span>
                        <div class="flex items-center justify-center gap-2 relative z-10 font-mono">
                            <span class="text-3xl font-black text-slate-800 dark:text-white leading-none text-start">{{ number_format($service->price, 2) }}</span>
                            <span class="text-xs font-black text-slate-400 font-Cairo text-start">USD</span>
                        </div>
                    </div>
                    ...
                    <div class="px-8 py-5 bg-amber-500/[0.03] rounded-[2.5rem] border border-amber-500/10 text-center relative group overflow-hidden">
                        ...
                        <span class="block text-[8px] font-black text-amber-600 uppercase tracking-[0.3em] mb-2 font-Cairo relative z-10">{{ __('تقييم الخدمة') }}</span>
                        <div class="flex items-center justify-center gap-3 relative z-10 font-mono">
                            <span class="text-3xl font-black text-slate-800 dark:text-white leading-none text-start">{{ number_format($service->average_rating, 1) }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-start">
                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('مزود الخدمة') }}</span>
                    <div class="flex items-center gap-3 text-start">
                        <div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-slate-500 font-Cairo">
                             {{ mb_substr($service->provider->name ?? '?', 0, 1) }}
                        </div>
                        <p class="text-[13px] font-black text-slate-800 dark:text-white font-Cairo text-start">{{ $service->provider->name ?? __('غير محدد') }}</p>
                    </div>
                </div>

                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('الحالة الإدارية') }}</span>
                    <div class="text-start">
                ...
                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('حالة التوافر') }}</span>
                    <div class="text-start">
                ...
                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('حالة التنشيط') }}</span>
                    <div class="text-start">
                         <p class="text-[13px] font-black flex items-center gap-2 font-Cairo text-start {{ $service->is_active ? 'text-indigo-600' : 'text-slate-400' }}">
                ...
                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('الخدمة الرئيسية') }}</span>
                    <p class="text-[13px] font-black text-slate-800 dark:text-white font-Cairo text-start">{{ $service->parent->name ?? __('خدمة رئيسية') }}</p>
                </div>

                <div class="space-y-3 text-start">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('التسعير حسب المسافة') }}</span>
                    <p class="text-[13px] font-black text-slate-800 dark:text-white font-Cairo text-start">{{ $service->distance_based_price ? __('مفعل: حسب المسافة الفاصلة') : __('ثابت: لا يتأثر بالموقع') }}</p>
                </div>
                ...
                @if ($service->distance_based_price)
                    <div class="space-y-3 p-4 bg-brand-primary/[0.03] rounded-2xl border border-brand-primary/10 text-start">
                        <span class="text-[8px] font-black text-brand-primary uppercase tracking-[0.3em] px-1 font-Cairo text-start">{{ __('معامل السعر / كم') }}</span>
                        <div class="flex items-baseline gap-1 font-mono text-start">
                             <span class="text-xl font-black text-slate-800 dark:text-white text-start">{{ number_format($service->price_per_km, 2) }}</span>
                             <span class="text-[10px] font-black text-slate-400 font-Cairo text-start">USD</span>
                        </div>
                    </div>
                @endif
            </div>
    ...
    <div class="card-premium glass-panel p-12 md:p-16 rounded-[4.5rem] shadow-2xl relative border border-white dark:border-slate-800/50 text-start">
        <div class="flex items-center gap-4 mb-10 text-start">
            ...
            <h4 class="text-2xl font-black text-slate-800 dark:text-white font-Cairo text-start">{{ __('وصف الخدمة والمهام') }}</h4>
        </div>
        <div class="relative text-start">
            <div class="absolute right-0 top-0 bottom-0 w-2 bg-slate-100 dark:bg-slate-800 rounded-full"></div>
            <p class="text-lg font-black text-slate-500 dark:text-slate-400 leading-[2] pr-10 font-Cairo italic text-start">
    ...
    @if ($service->children->isNotEmpty())
        <div class="space-y-10 text-start">
            <div class="flex items-center gap-4 px-6 text-start">
                ...
                <h4 class="text-2xl font-black text-slate-800 dark:text-white font-Cairo text-start">{{ __('الخدمات الفرعية التابعة') }}</h4>
            </div>
                    ...
                    <div class="card-premium glass-panel p-10 rounded-[3rem] shadow-xl hover:-translate-y-2 transition-all duration-500 group border border-white dark:border-slate-800/50 text-start">
                        <div class="flex flex-col gap-6 text-start">
                            <div class="flex justify-between items-start gap-4 text-start">
                                <h5 class="text-lg font-black text-slate-800 dark:text-white group-hover:text-brand-primary transition-colors leading-[1.4] font-Cairo text-start">{{ $child->name }}</h5>
                                <div class="flex flex-col items-end text-start">
                                    <span class="text-xl font-black text-brand-primary font-mono text-start">{{ number_format($child->price, 2) }}</span>
                                    <span class="text-[8px] font-black text-slate-400 uppercase font-Cairo text-start">USD</span>
                                </div>
                            </div>
                            <div class="pt-6 border-t border-slate-50 dark:border-slate-800/50 flex items-center justify-between text-start">
                                <a href="{{ route('services.show', $child->id) }}" class="text-[9px] font-black text-slate-400 hover:text-brand-primary uppercase tracking-[0.3em] flex items-center gap-3 transition-colors font-Cairo text-start font-Cairo">
                                    {{ __('عرض التفاصيل') }}
