...
<div class="space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            ...
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2 mr-16 text-start font-Cairo">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 font-Cairo text-start">
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-3 font-Cairo">{{ __('إجمالي الخدمات') }}</span>
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['total'], 2, '0', STR_PAD_LEFT) }}</span>
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['available'], 2, '0', STR_PAD_LEFT) }}</span>
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['unavailable'], 2, '0', STR_PAD_LEFT) }}</span>
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['active'], 2, '0', STR_PAD_LEFT) }}</span>
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['meeting_service'], 2, '0', STR_PAD_LEFT) }}</span>
        ...
        <div class="card-premium glass-panel p-6 rounded-[2rem] shadow-xl border border-white dark:border-slate-800/50 relative overflow-hidden group text-start">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-3xl font-black text-slate-800 dark:text-white leading-none font-mono">{{ str_pad($stats['custom_service'], 2, '0', STR_PAD_LEFT) }}</span>
    ...
    <div class="card-premium glass-panel rounded-[3rem] overflow-hidden shadow-2xl relative border border-white dark:border-slate-800/50 text-start font-Cairo">
        <div class="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/40 text-start">
            <div class="flex items-center gap-4 text-start">
                ...
                <h4 class="font-black text-xl text-slate-800 dark:text-white font-Cairo text-start">{{ __('سجل الخدمات المتاحة') }}</h4>
            </div>
            <div class="flex items-center gap-3 text-start">
                <span class="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest border border-slate-200 dark:border-slate-700 font-Cairo text-start">
        ...
        <div class="overflow-x-auto text-start">
            <table class="w-full text-sm text-start">
                <thead class="bg-slate-50/30 dark:bg-slate-900/20 text-slate-400 font-black text-[9px] uppercase tracking-[0.2em] border-b border-slate-100 dark:border-slate-800 text-start">
                    <tr>
                    ...
                    @forelse ($services as $service)
                        <tr class="hover:bg-brand-primary/[0.03] transition-all group/row">
                            <td class="px-10 py-6 text-start">
                                <span class="font-black text-slate-800 dark:text-white text-base group-hover/row:text-brand-primary transition-colors font-Cairo text-start">{{ $service->name }}</span>
                            </td>
                            <td class="px-10 py-6 text-start">
                                <div class="flex items-center gap-3 text-start">
                                    <div class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-black text-slate-400 font-Cairo">
                                        {{ mb_substr($service->provider->name ?? '?', 0, 1) }}
                                    </div>
                                    <span class="text-xs font-black text-slate-600 dark:text-slate-400 font-Cairo text-start">{{ $service->provider->name ?? __('غير محدد') }}</span>
                                </div>
                            </td>
                            <td class="px-10 py-6 text-start">
                                <span class="px-3 py-1.5 bg-slate-50 dark:bg-slate-800/50 text-slate-400 border border-slate-100 dark:border-slate-700 rounded-xl text-[9px] font-black font-Cairo text-start">
                                    {{ $service->category->name ?? __('بدون قسم') }}
                                </span>
                            </td>
                            <td class="px-10 py-6 text-start">
                                <div class="flex items-center gap-1.5 text-start font-Cairo">
                                    <span class="text-base font-black text-brand-primary font-mono text-start">{{ number_format($service->price, 2) }}</span>
                                    <span class="text-[9px] font-black text-slate-400 uppercase font-Cairo text-start">USD</span>
                                </div>
                            </td>
                            <td class="px-10 py-6 text-start">
                                <div class="text-start">
                            ...
                            <td class="px-10 py-6 text-start">
                                <div class="text-start">
                                    <span class="inline-flex items-center px-4 py-1.5 rounded-full text-[9px] font-black {{ $service->is_active ? 'bg-indigo-500/10 text-indigo-600 border border-indigo-500/20' : 'bg-slate-100 text-slate-400 border border-slate-200' }} font-Cairo text-start">
                        ...
                        <tr>
                            <td colspan="7" class="px-10 py-32 text-center text-start">
                                <div class="flex flex-col items-center justify-center opacity-40 text-start">
                                    ...
                                    <h5 class="text-xl font-black text-slate-800 dark:text-white font-Cairo">{{ __('لا توجد خدمات متاحة حالياً') }}</h5>
                                    <p class="text-[10px] font-black text-slate-400 mt-4 max-w-xs leading-relaxed uppercase tracking-[0.2em] font-Cairo">
