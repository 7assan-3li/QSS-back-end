...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        ...
        <a href="{{ route('verification-packages.create') }}" class="group relative px-10 py-5 bg-slate-950 dark:bg-brand-primary rounded-[2rem] text-white text-[14px] font-black uppercase tracking-[0.3em] shadow-[0_20px_40px_-10px_rgba(0,0,0,0.4)] hover:scale-[1.05] transition-all font-Cairo flex items-center gap-4 overflow-hidden">
             <div class="absolute inset-0 bg-[var(--glass-bg)]/10 translate-y-full group-hover:translate-y-0 transition-transform duration-500"></div>
             <span class="relative z-10 w-6 h-6 bg-[var(--glass-bg)]/20 rounded-lg flex items-center justify-center group-hover:rotate-180 transition-transform font-Cairo">➕</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl mx-auto font-Cairo text-start">
        ...
        <div class="card-premium glass-panel p-8 rounded-[2rem] shadow-2xl border border-emerald-500/20 flex items-center gap-8 group hover:scale-[1.02] transition-all text-start bg-emerald-50/20 dark:bg-emerald-950/10">
            ...
            <div class="flex flex-col text-start">
                ...
                <span class="text-[12px] font-black text-emerald-500/60 uppercase tracking-[0.2em] mt-2 font-Cairo text-start">{{ __('الباقات النشطة حالياً') }}</span>
            </div>
        </div>
    </div>
    ...
    @if ($packages->count())
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 font-Cairo">
            @foreach ($packages as $package)
                <div class="card-premium glass-panel p-10 rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] relative border border-[var(--glass-border)] flex flex-col group overflow-hidden text-start hover:shadow-brand-primary/[0.05]">
                    ...
                    <div class="flex justify-between items-start mb-10 relative z-10 text-start">
                        ...
                        <div class="w-12 h-12 bg-[var(--main-bg)] rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-brand-primary transition-all shadow-inner font-Cairo">
                    ...
                    <div class="mb-10 relative z-10 text-start">
                        ...
                        <div class="flex items-baseline gap-2 text-start">
                            ...
                            <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-widest font-Cairo text-start">{{ __('ريال يمني') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 mb-12 flex-grow relative z-10 text-start font-Cairo">
                        ...
                        <div class="flex items-center gap-4 text-emerald-500/60 text-start font-Cairo">
                            ...
                            <span class="text-[13px] font-black uppercase tracking-[0.2em] font-Cairo text-start">{{ __('الصلاحية') }}: {{ $package->duration_days }} {{ __('يوم') }}</span>
                        </div>
                    </div>

                    <!-- Operational Controls -->
                    <div class="grid grid-cols-2 gap-5 relative z-10 text-start font-Cairo">
                        <a href="{{ route('verification-packages.show', $package->id) }}" class="py-4 bg-[var(--main-bg)] text-[var(--text-secondary)] text-[var(--main-text)] rounded-2xl text-[12px] font-black text-center hover:bg-slate-950 hover:text-white hover:bg-[var(--glass-border)] dark:hover:text-slate-950 transition-all font-Cairo uppercase tracking-widest border border-[var(--glass-border)] shadow-sm text-start">{{ __('عرض التفاصيل') }}</a>
                        <a href="{{ route('verification-packages.edit', $package->id) }}" class="py-4 bg-brand-primary text-white rounded-2xl text-[12px] font-black text-center shadow-2xl shadow-brand-primary/20 hover:scale-[1.03] transition-all font-Cairo uppercase tracking-widest text-start">{{ __('تعديل') }}</a>
                    </div>
                </div>
            @endforeach
        </div>
    @else
        <div class="py-32 bg-[var(--glass-bg)]/[0.3] /[0.2] rounded-[3rem] text-center border-2 border-dashed border-[var(--glass-border)] font-Cairo text-start">
            <div class="w-24 h-24 bg-[var(--main-bg)] rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 text-slate-300 shadow-inner animate-pulse font-Cairo">📦</div>
            <span class="text-[var(--text-muted)] font-black text-xs uppercase tracking-[0.3em] font-Cairo opacity-50 text-start">{{ __('لا توجد باقات مضافة حالياً.') }}</span>
        </div>
    @endif
</div>
@endsection
