    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            <h3 class="font-black text-3xl text-[var(--main-text)] flex items-center gap-4 text-start font-Cairo">
                <span class="w-16 h-16 bg-amber-500/10 rounded-2xl flex items-center justify-center text-amber-600 text-3xl font-Cairo shadow-lg shadow-amber-500/5 font-Cairo whitespace-nowrap inline-flex items-center justify-center">✍️</span>
        ...
        <a href="{{ route('verification-packages.show', $package->id) }}" class="w-14 h-14 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-2xl flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-sm border border-[var(--glass-border)] font-Cairo">
    ...
    <form action="{{ route('verification-packages.update', $package->id) }}" method="POST" class="space-y-12 text-start font-Cairo">
        ...
        <div class="card-premium glass-panel p-10 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
            ...
            <div class="flex items-center gap-5 mb-14 text-start font-Cairo">
                ...
                <h4 class="text-xl font-black text-[var(--main-text)] font-Cairo text-start italic">{{ __('تعديل البيانات الأساسية') }}</h4>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10 text-start font-Cairo">
                ...
                <div class="flex flex-col gap-4 text-start">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="text" name="name" value="{{ old('name', $package->name) }}" placeholder="{{ __('أدخل اسم الباقة الجديد...') }}" class="w-full bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[1.2rem] px-8 py-5 text-sm font-black text-[var(--main-text)] text-[var(--main-text)] focus:border-amber-500 focus:ring-[12px] focus:ring-amber-500/5 transition-all outline-none shadow-inner font-Cairo text-start" required>
                ...
                <div class="flex flex-col gap-4 text-start font-Cairo">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="number" step="0.01" name="price" value="{{ old('price', $package->price) }}" placeholder="0.00" class="w-full bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[1.2rem] px-8 py-5 text-sm font-black text-[var(--main-text)] text-[var(--main-text)] focus:border-emerald-500 focus:ring-[12px] focus:ring-emerald-500/5 transition-all outline-none shadow-inner font-Cairo font-mono text-start" required>
                ...
                <div class="flex flex-col gap-4 text-start font-Cairo">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="number" name="duration_days" value="{{ old('duration_days', $package->duration_days) }}" placeholder="30" class="w-full bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[1.2rem] px-8 py-5 text-sm font-black text-[var(--main-text)] text-[var(--main-text)] focus:border-brand-primary focus:ring-[12px] focus:ring-brand-primary/5 transition-all outline-none shadow-inner font-Cairo font-mono text-start" required>
        ...
        <div class="card-premium glass-panel p-10 rounded-[2rem] shadow-2xl relative border border-[var(--glass-border)] text-start font-Cairo">
            <div class="flex items-center gap-5 mb-10 text-start font-Cairo">
                ...
                <h4 class="text-xl font-black text-[var(--main-text)] font-Cairo text-start italic">{{ __('تعديل الوصف والمميزات') }}</h4>
            </div>

            <div class="flex flex-col gap-4 text-start font-Cairo">
                <label class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] px-3 font-Cairo text-start">{{ __('الوصف') }}</label>
                <textarea name="description" rows="6" placeholder="{{ __('تعديل وصف الباقة ومميزاتها...') }}" class="w-full bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[1.5rem] px-10 py-8 text-sm font-bold text-[var(--main-text)] text-[var(--main-text)] focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all outline-none resize-none leading-relaxed font-Cairo shadow-inner text-start italic">{{ old('description', $package->description) }}</textarea>
        ...
        <div class="card-premium glass-panel p-12 rounded-[2.5rem] shadow-2xl relative border border-[var(--glass-border)] flex flex-col md:flex-row items-center justify-between gap-12 overflow-hidden text-start font-Cairo">
            ...
            <div class="flex items-center gap-8 relative z-10 text-start font-Cairo">
                <div class="w-20 h-20 bg-[var(--glass-bg)] rounded-[1.5rem] flex items-center justify-center text-brand-primary shadow-xl border border-[var(--glass-border)] font-Cairo">
                ...
                <div class="flex flex-col text-start font-Cairo">
                    <h4 class="text-xl font-black text-[var(--main-text)] font-Cairo leading-none mb-3 text-start italic">{{ __('حالة تفعيل الباقة') }}</h4>
