    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            <h3 class="font-black text-3xl text-slate-800 dark:text-white flex items-center gap-4 text-start font-Cairo">
                <span class="w-16 h-16 bg-brand-primary/10 rounded-2xl flex items-center justify-center text-brand-primary text-3xl font-Cairo shadow-lg shadow-brand-primary/5 font-Cairo">➕</span>
        ...
        <a href="{{ route('verification-packages.index') }}" class="w-14 h-14 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-2xl flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-sm border border-slate-200 dark:border-slate-800 font-Cairo">
    ...
    <form action="{{ route('verification-packages.store') }}" method="POST" class="space-y-12 text-start font-Cairo">
        ...
        <div class="card-premium glass-panel p-12 rounded-[4rem] shadow-2xl relative border border-white dark:border-slate-800/50 overflow-hidden text-start font-Cairo">
            ...
            <div class="flex items-center gap-5 mb-14 text-start font-Cairo">
                ...
                <h4 class="text-xl font-black text-slate-800 dark:text-white font-Cairo text-start italic">{{ __('بيانات الباقة الأساسية') }}</h4>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10 text-start font-Cairo">
                ...
                <div class="flex flex-col gap-4 text-start">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="{{ __('أدخل اسم الباقة...') }}" 
                            class="w-full bg-slate-50/50 dark:bg-slate-950/40 border-2 border-slate-100 dark:border-slate-800 rounded-[1.5rem] px-8 py-5 text-sm font-black text-slate-700 dark:text-white focus:border-brand-primary focus:ring-[12px] focus:ring-brand-primary/5 transition-all outline-none shadow-inner font-Cairo text-start" required>
                ...
                <div class="flex flex-col gap-4 text-start font-Cairo">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="number" step="0.01" name="price" value="{{ old('price') }}" placeholder="0.00" 
                            class="w-full bg-slate-50/50 dark:bg-slate-950/40 border-2 border-slate-100 dark:border-slate-800 rounded-[1.5rem] px-8 py-5 text-sm font-black text-slate-700 dark:text-white focus:border-emerald-500 focus:ring-[12px] focus:ring-emerald-500/5 transition-all outline-none shadow-inner font-Cairo font-mono text-start" required>
                ...
                <div class="flex flex-col gap-4 text-start font-Cairo">
                    ...
                    <div class="relative group text-start font-Cairo">
                        <input type="number" name="duration_days" value="{{ old('duration_days', 30) }}" placeholder="30" 
                            class="w-full bg-slate-50/50 dark:bg-slate-950/40 border-2 border-slate-100 dark:border-slate-800 rounded-[1.5rem] px-8 py-5 text-sm font-black text-slate-700 dark:text-white focus:border-amber-500 focus:ring-[12px] focus:ring-amber-500/5 transition-all outline-none shadow-inner font-Cairo font-mono text-start" required>
        ...
        <div class="card-premium glass-panel p-12 rounded-[4rem] shadow-2xl relative border border-white dark:border-slate-800/50 text-start font-Cairo">
            <div class="flex items-center gap-5 mb-10 text-start font-Cairo">
                ...
                <h4 class="text-xl font-black text-slate-800 dark:text-white font-Cairo text-start italic">{{ __('وصف الباقة ومميزاتها') }}</h4>
            </div>

            <div class="flex flex-col gap-4 text-start font-Cairo">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] px-3 font-Cairo text-start">{{ __('الوصف') }}</label>
                <textarea name="description" rows="6" placeholder="{{ __('اكتب وصفاً مختصراً للباقة ومميزاتها...') }}" 
                    class="w-full bg-slate-50/50 dark:bg-slate-950/40 border-2 border-slate-100 dark:border-slate-800 rounded-[2.5rem] px-10 py-8 text-sm font-bold text-slate-700 dark:text-white focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all outline-none resize-none leading-relaxed font-Cairo shadow-inner text-start italic">{{ old('description') }}</textarea>
        ...
        <div class="card-premium glass-panel p-12 rounded-[4.5rem] shadow-2xl relative border border-white dark:border-slate-800/50 flex flex-col md:flex-row items-center justify-between gap-12 overflow-hidden text-start font-Cairo">
            ...
            <div class="flex items-center gap-8 relative z-10 text-start font-Cairo">
                <div class="w-20 h-20 bg-white dark:bg-slate-900 rounded-[2rem] flex items-center justify-center text-brand-primary shadow-xl border border-slate-100 dark:border-slate-800 font-Cairo">
                ...
                <div class="flex flex-col text-start font-Cairo">
                    <h4 class="text-xl font-black text-slate-800 dark:text-white font-Cairo leading-none mb-3 text-start italic">{{ __('تفعيل الباقة فوراً') }}</h4>
            ...
            <div class="flex flex-col items-center md:items-end gap-10 relative z-10 text-start font-Cairo">
                ...
                <div class="flex flex-wrap justify-center gap-6 text-start font-Cairo">
                    <button type="submit" class="px-12 py-5 bg-gradient-to-r from-brand-primary to-indigo-600 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-[0_25px_50px_-10px_rgba(var(--brand-primary-rgb),0.4)] hover:scale-[1.05] active:scale-95 transition-all duration-500 font-Cairo text-start">
                        {{ __('حفظ ونشر الباقة 💾') }}
                    </button>
                    <a href="{{ route('verification-packages.index') }}" class="px-12 py-5 bg-white dark:bg-slate-900 text-slate-400 rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.3em] hover:bg-slate-50 dark:hover:bg-slate-800 transition-all font-Cairo border border-slate-100 dark:border-slate-800 text-start">
                        {{ __('إلغاء') }}
                    </a>
                </div>
            </div>
