...
<div class="max-w-7xl mx-auto space-y-12 mt-4 animate-fade-in text-start font-Cairo">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            <h3 class="font-black text-3xl flex items-center gap-4 text-start font-Cairo">
                <span class="w-16 h-16 bg-pink-500/10 rounded-2xl flex items-center justify-center text-pink-600 text-3xl font-Cairo shadow-lg shadow-pink-500/5 whitespace-nowrap inline-flex items-center justify-center">⚖️</span>
        ...
        <div class="flex items-center gap-3 px-8 py-3 bg-pink-500/5 rounded-2xl border border-pink-500/10 text-pink-600 text-[13px] font-black uppercase tracking-[0.2em] font-Cairo shadow-sm animate-pulse">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 font-Cairo text-start">
        ...
        <div class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-[var(--glass-border)]">
            ...
            <div class="flex items-center gap-6 relative z-10 text-start">
                <div class="w-16 h-16 bg-[var(--glass-border)] rounded-2xl flex items-center justify-center text-[var(--text-muted)] shadow-inner group-hover:rotate-6 transition-transform font-Cairo text-2xl">📁</div>
                <div class="flex flex-col text-start">
                    <span class="text-3xl font-black font-mono leading-none text-start italic">{{ str_pad($stats['total'], 2, '0', STR_PAD_LEFT) }}</span>
                    <span class="text-[12px] font-black uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-60">{{ __('إجمالي الشكاوى') }}</span>
                </div>
            </div>
        </div>
        ...
        <a href="?status=in_progress" class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-blue-500/10 bg-blue-50/20 dark:bg-blue-950/10">
            ...
            <div class="flex items-center gap-6 relative z-10 text-start">
                ...
                <div class="flex flex-col text-start">
                    ...
                    <span class="text-[12px] font-black text-blue-500 uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-80">{{ __('قيد المعالجة') }}</span>
                </div>
            </div>
        </a>

        <!-- Resolved Complaints -->
        <a href="?status=resolved" class="card-premium glass-panel p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:scale-[1.05] transition-all text-start border border-emerald-500/10 bg-emerald-50/20 dark:bg-emerald-950/10">
            ...
            <div class="flex items-center gap-6 relative z-10 text-start">
                ...
                <div class="flex flex-col text-start">
                    ...
                    <span class="text-[12px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-2 font-Cairo text-start opacity-80">{{ __('بلاغات مكتملة') }}</span>
                </div>
            </div>
        </a>
    </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 text-start">
                ...
                <div class="flex gap-2 bg-slate-900/5 dark:bg-[var(--glass-bg)]/5 p-1.5 rounded-2xl border border-[var(--glass-border)] text-start font-Cairo shadow-inner">
                    @foreach([7, 30, 90] as $d)
                        <a href="?days={{ $d }}" class="px-6 py-2 rounded-xl text-[12px] font-black uppercase tracking-[0.2em] transition-all font-Cairo {{ $days == $d ? 'bg-pink-600 text-white shadow-lg shadow-pink-600/20' : 'opacity-40 hover:opacity-100' }}">
                            {{ $d }} يوم
                        </a>
                    @endforeach
                </div>
    ...
    <div class="card-premium glass-panel rounded-[4rem] overflow-hidden shadow-2xl relative border border-[var(--glass-border)] text-start font-Cairo">
        <div class="px-12 py-10 border-b border-[var(--glass-border)] bg-[var(--main-bg)] flex justify-between items-center text-start font-Cairo">
            <div class="text-start">
                ...
                <p class="text-[13px] font-black uppercase tracking-[0.3em] mt-2 text-start font-Cairo opacity-60">{{ __('قائمة بجميع الشكاوى المقدمة بخصوص طلبات الخدمات.') }}</p>
            </div>
            
            <a href="{{ route('request-complaints.export', request()->all()) }}" class="bg-brand-primary text-white px-8 py-3.5 rounded-2xl font-black text-[13px] uppercase tracking-widest flex items-center gap-4 hover:scale-105 transition-all shadow-xl shadow-brand-primary/20">
        ...
        <div class="overflow-x-auto text-start">
            <table id="complaints-table" class="w-full text-start">
                <thead class="bg-[var(--main-bg)] font-black text-[12px] uppercase tracking-[0.3em] border-b border-[var(--glass-border)] font-Cairo text-start opacity-60 uppercase">
                    <tr>
                ...
                <tbody class="divide-y divide-[var(--glass-border)] font-Cairo text-start">
                    @forelse ($complaints as $complaint)
                        <tr class="hover:bg-pink-500/[0.01] transition-all group text-start">
                            ...
                            <td class="px-10 py-7 text-start">
                                <a href="{{ route('requests.show', $complaint->request_id) }}" class="inline-flex items-center gap-3 px-4 py-2 bg-indigo-500/5 text-indigo-600 rounded-xl font-black text-[13px] border border-indigo-500/10 hover:bg-indigo-600 hover:text-white transition-all shadow-sm font-Cairo group/link">
                            ...
                            <td class="px-10 py-7 text-start">
                                <div class="flex flex-col gap-2 text-start">
                                    ...
                                    <span class="px-3 py-1 rounded-lg text-[14px] font-black uppercase tracking-widest w-max font-Cairo {{ $complaint->user_id == $complaint->request->user_id ? 'bg-pink-500/10 text-pink-600 border border-pink-500/20' : 'bg-indigo-500/10 text-indigo-600 border border-indigo-500/20' }} whitespace-nowrap inline-flex items-center justify-center">
                                        {{ $complaint->user_id == $complaint->request->user_id ? __('طرف العميل') : __('طرف المزود') }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-10 py-7 text-center">
                                <span class="px-4 py-2 rounded-xl text-[12px] font-black uppercase tracking-widest font-Cairo whitespace-nowrap inline-flex items-center justify-center @if($complaint->status == 'pending') bg-amber-500/10 text-amber-600 border border-amber-500/20 @elseif($complaint->status == 'in_progress') bg-blue-500/10 text-blue-600 border border-blue-500/20 @else bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 shadow-lg shadow-emerald-500/10 @endif">
                                    {{ __($complaint->status) }}
                                </span>
                            </td>
                            <td class="px-10 py-7 text-end">
                                <a href="{{ route('request-complaints.show', $complaint) }}" class="inline-flex items-center justify-center gap-4 px-8 py-3 bg-[var(--glass-bg)] text-[var(--main-text)] rounded-[1.2rem] text-[13px] font-black uppercase tracking-[0.2em] hover:bg-[var(--glass-border)] text-white hover:text-white hover:bg-[var(--glass-border)] transition-all shadow-xl shadow-slate-900/5 font-Cairo border border-[var(--glass-border)]">
                        ...
                        <tr>
                            <td colspan="6" class="px-10 py-32 text-center">
                                <div class="flex flex-col items-center justify-center gap-6 opacity-30">
                                    ...
                                    <span class="text-[13px] font-black uppercase tracking-[0.3em] font-Cairo opacity-60">{{ __('لا توجد بلاغات أو شكاوى حاليا.') }}</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
