    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-8 text-start">
        <div class="text-start">
            <div class="flex items-center gap-5 mb-5 text-start">
                <a href="{{ route('request-complaints.index') }}" class="w-14 h-14 bg-[var(--glass-border)] text-[var(--text-muted)] rounded-2xl flex items-center justify-center hover:bg-pink-600 hover:text-white transition-all shadow-sm border border-[var(--glass-border)]">
                ...
                <h3 class="font-black text-3xl flex items-center gap-4 text-start font-Cairo">
                    <span class="w-12 h-12 bg-pink-500/10 rounded-2xl flex items-center justify-center text-pink-600 text-2xl font-Cairo shadow-lg shadow-pink-500/5 whitespace-nowrap inline-flex items-center justify-center">⚖️</span>
            ...
            <div class="flex items-center gap-3 text-[13px] font-black mt-3 mr-24 uppercase tracking-[0.2em] font-Cairo text-start opacity-60">
                ...
                <svg class="w-2 h-2 rtl:rotate-0 ltr:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
                <span class="text-pink-600 opacity-100">{{ __('رقم الشكوى') }} #{{ str_pad($requestComplaint->id, 5, '0', STR_PAD_LEFT) }}</span>
            </div>
        </div>
        
        <div class="flex items-center gap-4">
             <span class="px-8 py-3 rounded-2xl text-[13px] font-black uppercase tracking-[0.1em] font-Cairo shadow-xl @if($requestComplaint->status == 'pending') bg-amber-500/10 text-amber-600 border border-amber-500/20 shadow-amber-500/5 @elseif($requestComplaint->status == 'in_progress') bg-blue-500/10 text-blue-600 border border-blue-500/20 shadow-blue-500/5 @else bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 shadow-emerald-500/5 @endif whitespace-nowrap inline-flex items-center justify-center">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-12 text-start font-Cairo">
            ...
            @foreach ($statusSteps as $step)
                ...
                <div class="flex-1 flex flex-col items-center gap-5 group relative text-center font-Cairo">
                    <div class="w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-all duration-700 relative z-10 font-Cairo @if($isCurrent) bg-pink-600 text-white shadow-[0_20px_40px_-5px_rgba(236,72,153,0.4)] scale-110 ring-8 ring-pink-500/10 @elseif($isCompleted) bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 @else bg-[var(--glass-border)] transition-colors border border-[var(--glass-border)] shadow-inner opacity-60 @endif font-black text-lg">
                        @if($isCompleted)
                    ...
                    <div class="flex flex-col items-center text-center font-Cairo">
                        <span class="text-[13px] font-black uppercase tracking-[0.2em] font-Cairo mb-2 @if($isCurrent) text-pink-600 @elseif($isCompleted) text-emerald-600 @else opacity-60 @endif">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 text-start font-Cairo">
        ...
        <div class="lg:col-span-8 space-y-12 text-start">
            <div class="card-premium glass-panel p-14 rounded-[4.5rem] shadow-2xl relative border border-[var(--glass-border)] overflow-hidden text-start font-Cairo">
                ...
                <div class="flex items-center gap-5 mb-14 text-start font-Cairo font-Cairo">
                    <span class="w-3 h-10 bg-pink-600 rounded-full shadow-lg shadow-pink-600/30"></span>
                    <h4 class="text-2xl font-black font-Cairo text-start italic">{{ __('تفاصيل الشكوى المقدمة') }}</h4>
                </div>
                ...
                <div class="bg-[var(--main-bg)] p-12 rounded-[3.5rem] border border-[var(--glass-border)] mb-16 relative group text-start font-Cairo shadow-inner">
                    <div class="absolute -top-8 -right-8 w-20 h-20 bg-[var(--glass-bg)] rounded-3xl shadow-2xl flex items-center justify-center text-4xl group-hover:rotate-12 transition-all duration-500 font-Cairo">⚖️</div>
                    <p class="text-xl font-bold leading-[2.2] font-Cairo italic text-start opacity-70">" {{ $requestComplaint->content }}"
                    </p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-12 text-start font-Cairo">
                    <!-- Petitioner Node (Reporter) -->
                    <div class="bg-[var(--glass-bg)]/40 p-10 rounded-[3rem] border border-[var(--glass-border)] shadow-xl group/node hover:scale-[1.02] transition-all text-start font-Cairo">
                        <span class="text-[12px] font-black uppercase tracking-[0.3em] block mb-6 font-Cairo text-start opacity-60 font-Cairo">{{ __('مقدم الشكوى') }}</span>
                        <div class="flex items-center gap-5 text-start">
                            ...
                            <div class="flex flex-col text-start font-Cairo">
                                <span class="text-base font-black font-Cairo leading-tight text-start italic">{{ $requestComplaint->user->name ?? 'NON-OBJECTIVE DATA' }}</span>
                                <span class="text-[12px] font-black uppercase tracking-widest mt-2 font-Cairo text-start p-1 bg-pink-500/10 text-pink-600 rounded-lg w-max">
                    ...
                    <div class="flex flex-col gap-8 justify-center text-start">
                        <div class="flex items-center gap-6 group/item text-start">
                            ...
                            <div class="flex flex-col text-start font-Cairo">
                                <span class="text-[12px] font-black uppercase tracking-[0.3em] mb-1 font-Cairo text-start opacity-60">{{ __('صاحب الطلب (العميل)') }}</span>
                                <span class="text-sm font-black font-Cairo text-start italic">{{ $requestComplaint->request->user->name ?? 'غير موجود' }}</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-6 group/item text-start">
                            ...
                            <div class="flex flex-col text-start font-Cairo">
                                <span class="text-[12px] font-black uppercase tracking-[0.3em] mb-1 font-Cairo text-start opacity-60">{{ __('مزود الخدمة (الطرف الآخر)') }}</span>
                                <span class="text-sm font-black font-Cairo text-start italic">{{ $requestComplaint->request->serviceProvider()->name ?? 'غير محدد' }}</span>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="mt-14 pt-10 border-t border-[var(--glass-border)] flex flex-col md:flex-row justify-between items-center gap-6 text-start">
                    <span class="inline-flex items-center gap-3 px-6 py-3 bg-[var(--main-bg)] rounded-2xl border border-[var(--glass-border)] text-[13px] font-black font-mono opacity-60 whitespace-nowrap inline-flex items-center justify-center">
                        <span class="w-2 h-2 rounded-full bg-slate-300"></span>
                        {{ __('وقت تقديم الشكوى') }}: {{ $requestComplaint->created_at->format('Y-m-d H:i') }}
                    </span>
                    <a href="{{ route('requests.show', $requestComplaint->request_id) }}" class="inline-flex items-center gap-3 px-6 py-3 bg-indigo-600 text-white rounded-2xl text-[13px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 hover:scale-105 transition-all font-Cairo text-start">
        ...
        <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
            <div class="card-premium glass-panel p-10 rounded-[3.5rem] shadow-2xl border border-[var(--glass-border)] text-start font-Cairo overflow-hidden relative">
                ...
                <div class="flex items-center gap-4 mb-10 text-start font-Cairo">
                    ...
                    <h4 class="font-black font-Cairo text-sm uppercase tracking-[0.2em] text-start italic">{{ __('اتخاذ إجراء بالشكوى') }}</h4>
                </div>
                ...
                <form method="POST" action="{{ route('request-complaints.update-status', $requestComplaint) }}" class="space-y-10 text-start font-Cairo">
                    ...
                    <div class="space-y-4 text-start font-Cairo">
                        <label class="text-[13px] font-black uppercase tracking-[0.3em] px-3 font-Cairo text-start opacity-60">{{ __('تعديل حالة الشكوى') }}</label>
                        <div class="relative text-start font-Cairo">
                            <select name="status" class="w-full px-10 py-6 bg-[var(--main-bg)] border-2 border-[var(--glass-border)] rounded-[2.5rem] text-sm font-black outline-none focus:border-pink-600 focus:ring-[12px] focus:ring-pink-500/5 appearance-none font-Cairo transition-all text-center shadow-inner">
                    ...
                    <div class="pt-6 text-start font-Cairo">
                        <button type="submit" class="w-full py-6 bg-gradient-to-r from-pink-600 to-rose-700 text-white rounded-[2.5rem] text-[14px] font-black uppercase tracking-[0.3em] shadow-[0_25px_50px_-10px_rgba(236,72,153,0.4)] hover:scale-[1.03] hover:shadow-pink-600/50 transition-all duration-500 font-Cairo flex items-center justify-center gap-4">
                            {{ __('تثبيت الحالة وحفظ التعديلات') }} ⚖️
                        </button>
                    </div>
                </form>
                
                <p class="mt-8 text-[12px] font-black text-center leading-relaxed font-Cairo uppercase tracking-widest px-6 opacity-60">
                    {{ __('ملاحظة: سيتم حفظ هذا القرار وتحديث حالة الشكوى في سجلات النظام بشكل نهائي.') }}
                </p>
            </div>
            
            <!-- Audit Tip Card -->
            <div class="p-8 bg-[var(--glass-bg)] text-[var(--main-text)] border border-[var(--glass-border)] rounded-[3rem] shadow-2xl relative overflow-hidden text-start font-Cairo">
                <div class="absolute top-0 right-0 w-24 h-24 bg-[var(--glass-bg)]/5 rounded-bl-[5rem] -mr-10 -mt-10"></div>
                <h5 class="text-[13px] font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-3 font-Cairo text-start">
                    <span class="w-2 h-2 bg-emerald-500 rounded-full"></span>
                    {{ __('إرشادات المراجعة') }}
                </h5>
                <p class="text-[14px] font-bold text-[var(--text-muted)] leading-relaxed font-Cairo text-start">
