...
<body class="font-Cairo antialiased min-h-screen bg-[var(--glass-bg)] dark:bg-black overflow-hidden selection:bg-brand-primary selection:text-white">
    ...
    <div class="flex min-h-screen flex-col lg:flex-row">
        ...
        <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden mesh-gradient items-center justify-center p-20 order-last lg:order-first">
            ...
            <div class="absolute top-[-10%] left-[-10%] w-96 h-96 bg-[var(--glass-bg)]/10 rounded-full blur-3xl animate-pulse"></div>
            ...
            <div class="relative z-10 text-center flex flex-col items-center animate-fade-slide-up">
                ...
                <div class="mt-16 flex items-center gap-8">
                    <div class="flex flex-col items-center gap-1 opacity-60">
                        ...
                        <span class="text-[13px] uppercase tracking-widest text-white/70">{{ __('دعم فني مستمر') }}</span>
                    </div>
                    {{-- <div class="w-px h-10 bg-[var(--glass-bg)]/20"></div>
                    <div class="flex flex-col items-center gap-1 opacity-60">
                        <span class="text-2xl font-black text-white">AES-256</span>
                        <span class="text-[13px] uppercase tracking-widest text-white/70">{{ __('تشفير البيانات') }}</span>
                    </div> --}}
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col p-8 lg:p-12 relative bg-[var(--main-bg)]">
            ...
            <div class="absolute top-8 ltr:right-8 rtl:left-8 flex items-center gap-4 animate-fade-slide-up delay-400">
                <div class="flex items-center bg-[var(--glass-bg)] border border-[var(--glass-border)] p-1 rounded-2xl shadow-xl">
                    ...
                    <div class="w-px h-5 bg-[var(--glass-bg)] dark:bg-[var(--glass-bg)]/10 mx-1"></div>
                    <a href="{{ route('lang.switch', app()->getLocale() == 'ar' ? 'en' : 'ar') }}" class="w-11 h-11 flex items-center justify-center rounded-xl font-black text-[13px] hover:bg-[var(--glass-bg)] dark:hover:bg-[var(--glass-bg)]/10 hover:text-brand-primary transition-all duration-500 italic uppercase">
            ...
            <div class="flex-1 flex flex-col items-center justify-center">
                ...
                <div class="w-full max-w-md space-y-12 animate-fade-slide-up delay-100">
                    <div class="text-start lg:text-start">
                        ...
                        <p class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] font-Cairo">{{ __('مرحباً بك، يرجى إدخال بياناتك للدخول.') }}</p>
                    </div>
                        ...
                        <div class="p-6 bg-rose-500/10 border border-rose-500/20 rounded-[2rem] animate-shake">
                            <ul class="space-y-1">
                                @foreach ($errors->all() as $error)
                                    <li class="text-[14px] font-black text-rose-600 dark:text-rose-400 flex items-center gap-3">
                    ...
                    <form method="POST" action="{{ route('login.store') }}" class="space-y-8">
                        ...
                        <div class="space-y-4 group">
                            <label for="email" class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] px-6 transition-colors group-focus-within:text-brand-primary italic">{{ __('البريد الإلكتروني') }}</label>
                            <div class="relative">
                                <input type="email" id="email" name="email" value="{{ old('email') }}" required autofocus class="w-full py-6 px-16 bg-[var(--glass-bg)] border-2 border-[var(--glass-border)] rounded-[2rem] text-sm font-black outline-none transition-all focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 placeholder:text-slate-300 dark:placeholder:text-[var(--main-text)]"
                                       placeholder="">
                        ...
                        <div class="space-y-4 group">
                            <div class="flex justify-between items-center px-6">
                                <label for="password" class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] transition-colors group-focus-within:text-brand-primary italic">{{ __('كلمة المرور') }}</label>
                            </div>
                            <div class="relative">
                                <input type="password" id="password" name="password" required class="w-full py-6 px-16 bg-[var(--glass-bg)] border-2 border-[var(--glass-border)] rounded-[2rem] text-sm font-black outline-none transition-all focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 placeholder:text-slate-300 dark:placeholder:text-[var(--main-text)]"
                                       placeholder="">
                        ...
                        <div class="flex items-center justify-between px-6">
                            <label class="flex items-center gap-4 cursor-pointer group">
                                <div class="relative flex items-center">
                                    <input type="checkbox" name="remember" class="peer h-6 w-6 cursor-pointer appearance-none rounded-lg border-2 border-[var(--glass-border)] transition-all checked:bg-brand-primary checked:border-brand-primary">
                                ...
                                <span class="text-[13px] font-black text-[var(--text-muted)] uppercase tracking-widest group-hover:text-brand-primary transition-colors italic">{{ __('تذكرني') }}</span>
                            </label>
                        </div>

                        <!-- Submit Button -->
                        <div class="pt-10">
                            <button type="submit" class="w-full py-6 bg-gradient-to-tr from-brand-primary to-indigo-600 dark:from-brand-primary dark:to-indigo-500 text-white rounded-[2.2rem] text-[14px] font-black uppercase tracking-[0.4em] shadow-2xl hover:shadow-brand-primary/40 hover:scale-[1.02] hover:-translate-y-1 transition-all duration-500 flex items-center justify-center gap-4 group">
                                <span>{{ __('دخول') }}</span>
                    ...
                    <div class="pt-20 text-center lg:text-start flex flex-col gap-2">
                        <p class="text-[12px] font-black text-slate-300 dark:text-[var(--text-secondary)] uppercase tracking-widest italic">
                            {{ __('حقوق النشر محفوظة لمنصة QSS') }}
                        </p>
                    </div>
                </div>
            </div>
