...
<div class="max-w-7xl mx-auto space-y-10 mt-4 animate-fade-in text-start font-Cairo pb-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div class="flex-1 space-y-6 text-center md:text-start font-Cairo">
                        <div class="space-y-2 text-start font-Cairo">
                            <div class="flex flex-wrap items-center gap-4 text-start">
                                <h2 class="text-4xl font-black font-Cairo italic text-start text-[var(--main-text)]">{{ $user->name }}</h2>
                                @php
                                ...
                                @endphp
                                <span class="px-5 py-2 rounded-xl text-[12px] font-black {{ $roleColor }} border uppercase tracking-widest font-Cairo shadow-sm italic transition-all whitespace-nowrap inline-flex items-center justify-center">
                                    {{ $roleInternalName }}
                                </span>
                            </div>
                            <p class="text-lg font-bold text-[var(--text-muted)] dark:text-[var(--text-muted)] italic">{{ $user->email }}</p>
                        </div>

                        <div class="flex flex-wrap items-center justify-start gap-8 pt-4">
                            <div class="flex flex-col gap-1">
                                <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-widest">{{ __('المرجع الرقمي') }}</span>
                            ...
                            <div class="flex flex-col gap-1">
                                <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-widest">{{ __('تم الانضمام') }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="bg-[var(--glass-bg)] p-10 rounded-[3rem] border border-[var(--glass-border)] shadow-xl space-y-8 relative overflow-hidden">
                    ...
                    <div class="flex items-center justify-between">
                        <h4 class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-widest italic">{{ __('المحفظة المالية') }}</h4>
                        <span class="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-600 font-bold italic whitespace-nowrap inline-flex items-center justify-center">💰</span>
                    </div>
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-widest block">{{ __('النقاط المشتراة') }}</span>
                            <span class="text-2xl font-black text-[var(--main-text)]">{{ number_format($user->paid_points) }}</span>
                        </div>
                        <div class="space-y-1">
                            <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-widest block">{{ __('نقاط المكافأة') }}</span>
                ...
                <div class="bg-[var(--glass-bg)] p-10 rounded-[3rem] border border-[var(--glass-border)] shadow-xl space-y-8 relative overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <span class="text-[12px] font-black text-[var(--text-muted)] uppercase tracking-widest block">{{ __('إجمالي الطلبات') }}</span>
                            <span class="text-2xl font-black text-[var(--main-text)]">{{ $user->requests_count }}</span>
                        </div>
                        <div class="space-y-1">
        ...
        <div class="space-y-10">
            ...
            <div class="bg-[var(--glass-bg)] border border-[var(--glass-border)] p-10 rounded-[3.5rem] shadow-xl space-y-8">
                <h4 class="text-[14px] font-black text-[var(--text-muted)] uppercase tracking-widest italic">{{ __('آخر طلبات الهوية') }}</h4>
