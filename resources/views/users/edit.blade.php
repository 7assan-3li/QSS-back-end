    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-12 text-start font-Cairo">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 text-start font-Cairo px-4">
            ...
            <div class="lg:col-span-8 space-y-12 text-start font-Cairo">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 text-start font-Cairo relative z-10">
                        <div class="space-y-4 text-start font-Cairo">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" required
                                   class="input-premium w-full px-10 py-6 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] text-sm font-black outline-none focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all dark:text-white font-Cairo shadow-sm italic text-start">
                        </div>
                        ...
                        <div class="space-y-4 text-start font-Cairo">
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" required
                                   class="input-premium w-full px-10 py-6 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] text-sm font-black outline-none focus:border-brand-primary focus:ring-[15px] focus:ring-brand-primary/5 transition-all dark:text-white font-mono tracking-widest text-start italic shadow-sm uppercase">
                        </div>
                ...
                <div class="card-premium glass-panel p-12 rounded-[4rem] shadow-2xl relative border border-white dark:border-white/5 overflow-hidden text-start font-Cairo">
                    ...
                    <div class="flex items-center gap-5 mb-14 text-start font-Cairo relative z-10">
                        ...
                        <h4 class="text-2xl font-black font-Cairo text-start italic text-slate-800 dark:text-white">{{ __('الملف المهني للمزود') }} (PROFESSIONAL_ID)</h4>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 text-start font-Cairo relative z-10">
                        <div class="space-y-4 md:col-span-2 text-start font-Cairo">
                            <label for="job_title" class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] px-3 opacity-60">
                                <span class="w-2 h-2 bg-emerald-500 rounded-full"></span>
                                {{ __('المسمى الوظيفي') }}
                            </label>
                            <input type="text" id="job_title" name="job_title" value="{{ old('job_title', $user->profile->job_title ?? '') }}"
                                   class="input-premium w-full px-10 py-6 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] text-sm font-black outline-none focus:border-brand-primary transition-all dark:text-white shadow-sm italic text-start">
                        </div>

                        <div class="space-y-4 md:col-span-2 text-start font-Cairo">
                            <label for="bio" class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] px-3 opacity-60">
                                <span class="w-2 h-2 bg-indigo-500 rounded-full"></span>
                                {{ __('السيرة الذاتية (Bio)') }}
                            </label>
                            <textarea id="bio" name="bio" rows="4"
                                   class="input-premium w-full px-10 py-8 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[3rem] text-sm font-black outline-none focus:border-brand-primary transition-all dark:text-white shadow-sm text-start font-Cairo leading-relaxed">{{ old('bio', $user->profile->bio ?? '') }}</textarea>
                        </div>

                        <div class="space-y-4 text-start font-Cairo">
                            <label for="latitude" class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] px-3 opacity-60">
                                <span class="w-2 h-2 bg-slate-400 rounded-full"></span>
                                {{ __('خط العرض (Latitude)') }}
                            </label>
                            <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $user->profile->latitude ?? '') }}"
                                   class="input-premium w-full px-10 py-6 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] text-sm font-black outline-none focus:border-brand-primary transition-all dark:text-white font-mono shadow-sm text-start">
                        </div>

                        <div class="space-y-4 text-start font-Cairo">
                            <label for="longitude" class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] px-3 opacity-60">
                                <span class="w-2 h-2 bg-slate-400 rounded-full"></span>
                                {{ __('خط الطول (Longitude)') }}
                            </label>
                            <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $user->profile->longitude ?? '') }}"
                                   class="input-premium w-full px-10 py-6 bg-white dark:bg-slate-950/50 border-2 border-slate-100 dark:border-white/5 rounded-[2.5rem] text-sm font-black outline-none focus:border-brand-primary transition-all dark:text-white font-mono shadow-sm text-start">
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-4 space-y-12 text-start font-Cairo">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 p-12 rounded-[4rem] shadow-xl relative overflow-hidden text-start font-Cairo">
                    <h4 class="font-black text-[10px] uppercase tracking-[0.4em] mb-12 px-2 text-start opacity-60 italic">💸 {{ __('الحوكمة والعمولات') }}</h4>
                    
                    <div class="space-y-10 relative z-10 text-start">
                        <div class="space-y-4">
                            <label for="commission" class="text-[9px] font-black text-slate-400 uppercase tracking-widest px-2">{{ __('نسبة العمولة المخصصة %') }}</label>
                            <div class="relative">
                                <input type="number" id="commission" name="commission" value="{{ old('commission', $user->commission ?? '10') }}"
                                       class="w-full px-8 py-5 bg-slate-50 dark:bg-white/5 rounded-2xl text-lg font-black text-brand-primary outline-none border-2 border-transparent focus:border-brand-primary/20 transition-all font-mono italic">
                                <span class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 font-mono font-black">%</span>
                            </div>
                        </div>

                        <div class="flex items-center justify-between p-6 bg-rose-500/5 rounded-3xl border border-rose-500/10">
                            <div class="flex flex-col gap-1">
                                <span class="text-[11px] font-black text-rose-600">{{ __('استثناء من العمولات') }}</span>
                                <span class="text-[8px] font-black text-rose-400 uppercase">{{ __('NO_COMMISSION_MODE') }}</span>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="no_commission" value="1" {{ $user->no_commission ? 'checked' : '' }} class="sr-only peer">
                                <div class="w-14 h-8 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:-translate-x-full rtl:peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:inline-start-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all dark:border-gray-600 peer-checked:bg-rose-500"></div>
                            </label>
                        </div>

                        <div class="space-y-4 pt-6">
                            <label for="provider_verified_until" class="text-[9px] font-black text-slate-400 uppercase tracking-widest px-2">{{ __('تاريخ انتهاء التوثيق') }}</label>
                            <input type="date" id="provider_verified_until" name="provider_verified_until" value="{{ $user->provider_verified_until ? \Carbon\Carbon::parse($user->provider_verified_until)->format('Y-m-d') : '' }}"
                                   class="w-full px-8 py-5 bg-slate-50 dark:bg-white/5 rounded-2xl text-xs font-black outline-none border-2 border-transparent focus:border-brand-primary/20 transition-all dark:text-white uppercase font-mono">
                        </div>
                    </div>
                </div>
                @endif
                ...
                <!-- Safety Hub -->
                <div class="card-premium glass-panel p-12 rounded-[4rem] shadow-2xl relative border border-white dark:border-white/5 overflow-hidden text-start font-Cairo">
                     <button type="submit" class="w-full py-7 bg-gradient-to-r from-brand-primary to-indigo-600 text-white rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl hover:shadow-brand-primary/40 hover:scale-[1.02] active:scale-95 transition-all duration-500 flex items-center justify-center gap-5 italic text-start">
        ...
        <div class="card-premium glass-panel p-12 rounded-[4rem] shadow-2xl relative border border-white dark:border-white/5 overflow-hidden text-start font-Cairo">
            ...
            <div class="flex items-center gap-5 mb-14 text-start font-Cairo relative z-10">
                ...
                <div class="text-start">
                    ...
                    <p class="text-[10px] font-black uppercase tracking-widest mt-2 opacity-50">{{ __('اترك الحقول فارغة إذا كنت لا تريد تغيير كلمة المرور.') }}</p>
                </div>
            </div>
            ...
            <form action="{{ route('users.update.password', $user->id) }}" method="POST" class="space-y-10 text-start font-Cairo relative z-10">
                ...
                <div class="space-y-4 text-start">
                    <label for="password" class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] px-3 opacity-60">
                ...
                <div class="pt-6 text-start">
                    <button type="submit" class="w-full py-7 bg-gradient-to-r from-amber-600 to-amber-500 text-white rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl hover:shadow-amber-500/40 hover:scale-[1.02] active:scale-95 transition-all duration-500 flex items-center justify-center gap-5 italic text-start">
